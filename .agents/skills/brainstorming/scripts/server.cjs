...
<html>
<head><meta charset="utf-8"><title>工程设计可视化伴侣</title>
<style>
...
<body><!-- BRANDING --><h1>工程设计可视化伴侣</h1>
<p>正在等待 Agent 推送设计页面...</p></body></html>`);
}
...
<body><h1>需要会话密钥</h1>
<p>请使用编码 Agent 提供的完整 URL 重新打开本页，其中必须包含 <code>?key=&hellip;</code>。</p></body></html>`;

function bootstrapPage(key) {
  const jsonKey = JSON.stringify(String(key));
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>正在打开工程设计可视化伴侣</title></head>
<body>
<script>
...
function brandMarkup() {
  ...
  const text = '工程设计可视化伴侣 v' + version;
  return '<div class="brand"><a href="https://github.com/obra/superpowers">' +
    '<span class="brand-copy">' + text + '</span></a></div>';
}
