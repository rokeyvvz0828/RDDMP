...
- “输入项配置/业务表单元数据”能力当前已下线。新业务默认由所属模块维护独立的前端页面、字段 DTO、校验、数据库结构和 Mock 数据，不得依赖 `biz_form_*` 表或输入项配置接口。`docs/integration/business-form-metadata-contract.md` 仅供存量兼容参考；重新启用必须有单独的 `ready` 需求。
