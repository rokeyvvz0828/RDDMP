...
- 每个需求使用独立工作区和短生命周期分支：`feat|fix|hotfix|docs|chore/REQ-YYYYMMDD-NNN-short-name`。唯一既有协作分支例外为精确小写值 `licon`；任务范围必须显式填写该值，且不得由此前缀、通配符、大小写变体或其他任意分支推导例外。
...
规约、模块边界、CODEOWNERS、CI 或仓库 Skill 变更必须关联状态为 `ready` 的需求编号及任务范围文件，更新唯一事实源并同步必要摘要。个人聊天记录、临时提示词和本地配置不构成团队规则。
