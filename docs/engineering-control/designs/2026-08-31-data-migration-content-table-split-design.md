...
## 10. 统一回收站查看明细增补（2026-09-02）

### 10.1 目标与范围

统一回收站 `/data-migration/content/recycle-bin` 在现有列表、恢复、彻底清理之外新增“查看”操作。用户点击后在当前页面打开只读抽屉，不离开回收站；本次不增加编辑、恢复前修改或下载行为。

### 10.2 用户流程与状态

1. 回收站列表每行显示“查看”操作；操作与恢复/清理互斥，提交中的行不可重复点击。
2. 点击“查看”先打开抽屉并显示加载状态，再请求统一详情接口；请求失败显示可重试错误，关闭抽屉不影响列表筛选、分页和选择状态。
3. 详情成功后按“基本信息、归属与审计、内容/附件、类型专属字段”分组展示。字段缺失或空值统一显示 `—`；结构化 JSON 以可读文本展示，不执行其中内容。
4. 详情接口只返回当前租户、指定类型且仍为软删除状态的记录。类型不支持、记录不存在、已恢复/已清理和无权限分别返回现有 `BAD_REQUEST`/`FORBIDDEN` 语义，不泄露其他租户数据。
5. 桌面端抽屉宽度不超过视口减 24px；移动端使用近全屏宽度、内容可滚动，关闭按钮和关键字段始终可操作。

### 10.3 后端契约

- `GET /api/data-migration/recycle-bin/{type}/{id}`，沿用统一回收站控制器的 `data-migration:manage` 或 `system:admin` 权限。
- 成功响应：`ApiResponse<Map<String,Object>>`；必须包含 `id`、`asset_type`、`asset_code`、`asset_name`、项目/归属字段、创建/更新时间、`deleted_by`、`deleted_at`，并保留来源的类型专属字段（文件元数据、`structured_data`、会议关联与附件等）。
- 服务层新增 `RecycleBinSource.detail(type,id,user)` SPI；各来源通过已有业务服务/SQL 使用租户条件和 `deleted=1` 查询，不复用仅查询活动记录的普通详情接口。
- 详情查询不写审计、不改变状态、不触发附件下载；恢复和彻底清理继续沿用原接口。

### 10.4 前端契约与验收

- `data-migration.ts` 新增 `getDataMigrationRecycleBinDetail(type,id)` 和 `DataMigrationContentRecycleDetail` 类型。
- `RecycleBinPage.vue` 增加详情状态（打开/加载/成功/失败/关闭）和只读抽屉；类型标签和字段渲染不得依赖后端返回的人类文案。
- 验收：至少覆盖文件型、结构化型、汇报材料、会议纪要四类；软删除记录可查看，活动记录/不存在 ID 被拒绝；403 状态不展示数据；桌面 1280x800 与移动 375x812 无横向溢出；前端构建和 data-migration 后端聚焦测试通过。
