...
- 修订 3：执行前采样确认历史 Docker 卷的初始化密码不可由环境变量覆盖；启动器改用固定 `rddmp-dev` Compose 项目、专用容器名和专用命名卷，Compose 原默认名称保持兼容。
- 修订 4：真实启动证明聚合根执行 `spring-boot:run` 找不到主类；后端热重载改为 reactor `install` 后从 boot POM 启动，文件变化后由 Node 自动重启后端，不引入 DevTools。
- 修订 5：Windows 空库运行证明本地固定工作流发布器会因业务前置条件终止应用，启动器将其关闭；同时替换无法通过当前 PasswordEncoder 校验的旧演示哈希。

## 目标与成功信号

研发人员从仓库根目录执行一个平台对应命令，即可启动固定、可重复的本地开发环境。基础设施就绪后，Flyway 自动迁移，管理员可以使用 `admin/admin123` 登录，前端修改由 Vite HMR 更新，后端修改经 reactor 安装后由 Node 编排器自动重启。
...
- 启动器关闭会修改业务状态且可能阻止空库启动的本地固定工作流发布器；Mock 数据初始化和 Flyway 保持启用。

### R4 前后端热重载

- 前端使用现有 `npm run dev` 和 Vite HMR。
- 编排器先执行 `mvn -pl :ccb-boot -am -DskipTests install`，再从 `server/src/platform/boot/pom.xml` 启动 Spring Boot。
- 编排器监听后端 `src/main/java`、`src/main/resources` 和相关 Maven POM；变更经防抖后串行执行 reactor `install`，成功后自动重启后端进程。
...
- 默认退出不删除容器或数据卷。
- 启动器不得复用或修改手工启动产生的 `ccb-platform-*` 容器和卷。
- 只修改任务范围列出的文件。
...
- `docker-compose.yml`：容器名增加带原值默认值的变量；启动器固定使用 `rddmp-dev-*`，Compose 项目为命名卷增加 `rddmp-dev_` 前缀，手工启动继续使用 `ccb-platform-*` 默认值。
- `server/src/platform/boot/pom.xml`：保持只读，作为具有 Spring Boot 主类的实际启动 POM。
...
3. Docker Compose 以 `COMPOSE_PROJECT_NAME=rddmp-dev`、专用容器和项目隔离卷启动三个基础设施服务；轮询容器内 `mysqladmin ping`。
4. 若 `sys_user` 已存在，使用容器内 MySQL 客户端执行带精确主键和租户条件的密码哈希更新；空库跳过。
5. Maven reactor 安装模块产物后，从 boot POM 以 `local` profile 启动后端，Spring Boot 自动运行 Flyway。
6. npm 启动 Vite；编排器并行等待后端健康检查。
7. 后端源码变化触发防抖 reactor 安装；成功后 Node 终止并重启后端进程。
...
- 后端增量安装失败：打印失败，不杀死当前可用后端；后续变化重新尝试。
...
- 文件监听使用单一防抖队列，安装期间的新变化合并为下一轮，避免并发 Maven 进程；只有安装成功才重启后端。
...
- 热重载：修改并恢复受控后端资源和前端文件，观察 reactor 安装、Node 后端重启与 Vite HMR 信号。
...
- 假设本地端口 13306、8080、5173、19000、19001、18012 可用；冲突时明确失败，不自动改端口以保持环境确定性。
...
- Maven reactor 安装耗时可能延长后端重载：采用防抖、单并发和仅成功后重启，回退启动脚本即可恢复手工启动。
