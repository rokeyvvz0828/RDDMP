...
只追加 V141；不改公共 UI 或平台附件；所有服务端写入验证 tenant/domain/project/system/RBAC 并审计；使用现有全局项目选择器、UiDataTable、TestManagementFormDialog、WangEditor 和附件网关。

## 文件职责地图

| 路径 | 状态 | 职责 |
|---|---|---|
| `.../V141__test_management_execution_defect.sql` | candidate-new | 执行、缺陷、关联、附件、轨迹、菜单动作表。 |
...
**回滚：** 回退 execution 包及控制器，不删除 V141 数据。
