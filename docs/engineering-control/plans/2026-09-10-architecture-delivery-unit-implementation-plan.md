...
# 计划修订 3（2026-09-11）：交付单元制品类型（字典驱动）

## 状态与来源
- 计划修订：3
- 设计修订：3
- 变更原因：用户要求为交付单元增加「制品类型」字段，按钮式单选、选项由统一字典管理；确认初始字典值为 容器/压缩包/脚本，其余按推荐（可选、可改、列表加列与筛选、单选）。

## 新增任务（与账本 `control-plan-3.json` 一致）

| 任务 | 目标 | 需求 | 关键检查 |
| --- | --- | --- | --- |
| T8 | 后端：`arch_delivery_unit.artifact_type_code` + `ARCH_ARTIFACT_TYPE` 字典种子 + 服务端字典校验 + 交付单元字典选项端点 + 列表筛选参数 | R9, R1, R2, R3, R8 | 启用项可写入；非法/停用 code 400；空值允许且存量兼容；选项端点白名单生效；可按 `artifactTypeCode` 筛选 |
| T9 | 前端：按钮式单选（`el-radio-group` + `el-radio-button`，含「未设置」）、列表列与筛选、详情展示 | R9, R8 | 构建通过；选项与字典一致；列表展示与筛选生效；移动视口可用 |
| T10 | 收敛：回归、范围审计、收敛证据更新 | R1—R9 | 架构模块测试全过；前端构建通过；浏览器验收全过；治理与范围检查无新增违规 |
