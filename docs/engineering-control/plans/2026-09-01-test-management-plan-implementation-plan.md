...
- 仅修改 REQ-20260831-057 的 `writable_paths`；新增迁移只能是 V139，不能修改 V137/V138 或平台附件/公共 UI。
...
| 路径 | 状态 | 职责 |
| --- | --- | --- |
| `server/src/platform/infrastructure/src/main/resources/db/migration/V139__test_management_plan.sql` | candidate-new | 方案、版本、专项节点、审计表及细粒度权限种子 |
...
- 新建：`server/src/platform/infrastructure/src/main/resources/db/migration/V139__test_management_plan.sql`
...
1. 建立迁移基线检查：运行 `node scripts/check-flyway-migrations.mjs`，记录新增 V139 前的结果。
2. 追加 V139：建立专项节点、逻辑方案、版本和审计表；创建 `(tenant,domain,project,node,name,deleted)` 查询索引，向四个 plans 菜单追加 create/update/delete 权限并只授予初始管理员。
...
**验收、证据与回滚：** 保存迁移与测试输出；可回退 T1 应用代码，已执行 V139 和软删除数据保留。
...
**验收、证据与回滚：** 保存 API 探针及测试输出；回退 T2 删除端点和策略注册，V139 数据保持。
...
- 执行器候选：V139、方案服务、访问策略、控制器、专属前端页面与 API DTO。
- 扰动候选：全局项目选择器状态、参测系统配置为空、MinIO/kkFileView 不可达、用户并行改动。

## 风险与用户批准

用户已经确认在当前分支实施本计划所描述的测试方案模块。高风险动作仅为追加 V139、权限种子和平台附件公开契约的消费；不修改平台实现。
