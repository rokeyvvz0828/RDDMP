...
2. 新增业务功能至少覆盖列表、详情/表单、成功状态、空状态或异常状态中的关键一组数据；涉及动态表单时，同时覆盖 scope、分区、字段、规则、选项和至少一条扩展值。
...
业务功能的 Agent 必须把表单元数据和业务 Mock 数据作为同一交付物维护。字段层级、来源、规则、版本和首次启动约束统一遵循 `docs/integration/business-form-metadata-contract.md`，本文件只补充 Mock 侧要求：

- `mock/mock-data.json` 至少登记对应功能目录、`biz_form_*` 元数据行和一条可查询的扩展字段值；
- `MockDataInitializer` 必须先完成表字段白名单、JSON 字段校验和幂等 upsert，再登记数据；
- 空数据库或重建本地数据库首次启动必须无需人工打开“输入项配置”即可呈现业务配置，重复启动不得产生重复配置。
