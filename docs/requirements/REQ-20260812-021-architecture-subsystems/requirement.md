...
响应使用 camelCase，选项不返回 `tenantId`、密码散列、头像对象键或其他管理字段。详情和列表可返回显示标签、`responsibleTeamValid` 和物理子系统创建人显示名称；物理新增/编辑请求不得包含联系人、电话、团队名称快照、租户、状态、删除或审计字段。
...
6. 物理子系统系统负责人可空；物理子系统不维护联系人或联系电话，新增/编辑请求、数据库表、Mock 数据和页面均不得包含物理联系人字段。
