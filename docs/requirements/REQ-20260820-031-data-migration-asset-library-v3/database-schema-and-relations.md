...
> 需求：`REQ-20260820-031`
> 当前 DDL 基线：仓库 Flyway 迁移至 `V183`（V181 跳号缺失；V179 删除 `dm_project`；V178 收敛唯一键直接建在业务列上、删除所有 `active_*` 生成列；V177 目标表/字段主键改为业务编号 `table_code`/`field_code`（`BIGINT`）；V182 组件改自然键主键、内容表 `component_id`→`system_code`、组件改 `enabled` 启停替代 `deleted` 软删）
...
> 文档范围：只分析数据迁移模块自有的 19 张 `dm_` 表；平台表（`pm_project`、`pm_project_member`、`arch_physical_subsystem`、`att_file`、`sys_user`、RBAC 表等）由所属模块持有，本文件不展开其列结构

## 1. 当前模型概览

数据迁移模块采用“内容一菜单一表”的模型：7 张文件型内容表、2 张结构化内容表共享统一的租户、项目、系统编号、业务编号、负责人、软删除和审计字段；文件型内容通过公共附件关系表绑定平台附件。中间表结构与目标表共用 `dm_target_table` 主表和 `dm_target_table_field` 字段表，通过 `table_category='INTERMEDIATE'` 区分。会议和问题是独立实体，分别通过会议-系统关系表、问题关联表表达多对多或多态关系。

当前最终模型包含 19 张 `dm_` 表，不含 `dm_intermediate_table`（V169 删除）与已废弃删除的 `dm_project`（V179 删除）。全部表为 InnoDB、`utf8mb4_unicode_ci`。项目主数据统一使用平台 `pm_project`。
...
数据访问采用 Spring `JdbcTemplate` 直接写 SQL，Service 类即数据访问层，行数据以 `Map<String, Object>` 或 `PageResult<Map<String, Object>>` 返回；无传统 JPA Entity / MyBatis Mapper / Repository 分层。
...
| 基础资料 | `dm_component` | 10 | 系统/组件清单，自然键主键，`enabled` 启停 | 0 | 不适用 |
...
活动行指 `deleted = 0`。`dm_component` 无 `deleted` 列，使用 `enabled = 1` 表示启用，不使用活动行统计。快照、关系表和操作日志没有 `deleted` 列，不使用活动行统计。

### 2.2 关联使用的其他模块表（仅列名，不展开列结构）

下表按“当前运行 SQL”“菜单/RBAC SQL”“平台契约内部事实源”区分，避免把平台表误认为数据迁移模块自有表。本文件不展开这些表的列结构，详见各所属模块文档。
...
| `system_code` | varchar(64) | NOT NULL，默认 '' | 项目内系统编号（当前项目 `dm_component` 活动记录的编号），项目级使用空串哨兵 |
...
| `tenant_id` | bigint | NOT NULL，默认 1 | 租户 ID |
| `project_id` | bigint | NOT NULL | 所属平台项目；所有列表和写操作的隔离键 |
| `system_code` | varchar(64) | NOT NULL，默认 '' | 项目内系统编号 |
...
### 3.4 `dm_dependency`（迁移过程依赖文件，14 列）
...
| `topic_type_code` | varchar(64) | NOT NULL，默认 `LEGACY` | 专题类型编码 |
| `topic_summary` | varchar(2000) | NOT NULL，默认 '' | 专题摘要 |
| `owner_id` | bigint | NOT NULL | 负责人用户 ID |
| `deleted` | tinyint | NOT NULL，默认 0 | 逻辑删除标记 |
| `deleted_by` | bigint | NULL | 删除人 |
| `deleted_at` | timestamp | NULL | 删除时间 |
...
> 约束/索引：`uk_dm_topic_code (tenant_id, project_id, doc_code)`；`idx_dm_topic_query (tenant_id, project_id, deleted, updated_at)`；`idx_dm_topic_owner (tenant_id, owner_id, deleted)`；`idx_dm_topic_granularity (tenant_id, project_id, granularity, deleted, updated_at)`；`idx_dm_topic_type (tenant_id, project_id, topic_type_code, deleted)`

### 3.7 `dm_release_drill`（投产及演练，14 列）
...
| 字段 | 类型 | 默认/约束 | 说明 |
| --- | --- | --- | --- |
| `id` | bigint | PK | 问题主键（应用层 `nextId()` 生成，非自增） |
| `tenant_id` | bigint | NOT NULL，默认 1 | 租户 ID |
| `project_id` | bigint | NOT NULL | 所属项目 |
...
### 3.16 `dm_component`（系统/组件清单，10 列，自然键主键）

| 字段 | 类型 | 默认/约束 | 说明 |
| --- | --- | --- | --- |
| `tenant_id` | bigint | NOT NULL，默认 1 | 租户 ID |
| `project_id` | bigint | NOT NULL | 所属项目 |
| `system_code` | varchar(64) | NOT NULL | 项目内系统编号，跨表关联系统的统一业务键；系统名称展示经本表按编号投影 `arch_physical_subsystem` |
| `enabled` | tinyint | NOT NULL，默认 1 | 启用标记，1 启用、0 停用；替代 `deleted` 软删，停用后业务关联仍可保留但活动清单排除 |
...
> 约束/索引：主键 `(tenant_id, project_id, system_code)`（自然键，无 `id` 列）；`idx_dm_component_enabled (tenant_id, project_id, enabled)`；`idx_dm_component_list (tenant_id, project_id, enabled, updated_at)`；无 `deleted` 列，使用 `enabled` 启停
...
| `system_code` | varchar(64) | NOT NULL，默认 '' | 系统维度（项目级快照使用空串哨兵） |
| `metric_code` | varchar(64) | NOT NULL | 指标编码 |
| `metric_value` | decimal(20,4) | NOT NULL，默认 0 | 指标值 |
| `created_at` | timestamp | NOT NULL，默认 CURRENT_TIMESTAMP | 创建时间 |

> 约束/索引：`uk_dm_snapshot (tenant_id, snapshot_date, project_id, system_code, metric_code)`
...
flowchart LR
    ...
    AF["att_file"]
    CMP["dm_component<br/>自然键 (tenant_id,project_id,system_code)"]
...
除 `dm_target_table_field` 按 `(tenant_id, table_code)` 引用 `dm_target_table` 的组合物理外键外，图中关系均由服务层通过租户、项目、删除状态/启用状态和业务类型校验实现，不建立跨模块物理外键。
...
| 组件 → 内容 | 一对多 | `system_code`，看板统计时限定 `dm_component.enabled = 1` 活动行 |
...
| 会议 ↔ 项目内系统/组件 | 多对多 | `dm_meeting_system`，按 `(project_id, system_code)` 关联 `dm_component` 启用行 |
| 问题 → 会议/目标表/字段 | 一对多多态 | `related_type + related_id` |
| 组件/目标表/问题 → 项目内系统/组件 | 多对一，可空 | 按 `(project_id, system_code)` 关联 `dm_component` 启用行，只维护编号不保存 `arch_physical_subsystem.id` |
...
| 专题材料 `/data-migration/content/topics` | `TopicService` | `dm_topic`、`dm_content_attachment`、`dm_operation_log` | 文件型内容，含 `granularity`/`topic_type_code`/`topic_summary` 域字段 |
...
| 系统/组件清单 `/data-migration/base/components` | `ProjectComponentService` | `dm_component`、9 张内容表、`dm_operation_log` | 系统名称经 `arch_physical_subsystem` 按编号投影，关联以项目内 `system_code` 为唯一键；`enabled` 启停替代软删 |
...
模块唯一键直接建在业务列上：软删行同样占用唯一名额；删除后需先从回收站彻底删除（purge）才能重建同名/同编号记录；恢复仍只与存量活动行冲突，冲突由数据库唯一键拒绝，服务层统一翻译为业务冲突。`dm_component` 无软删，使用自然键主键 + `enabled` 启停，停用后主键行仍占用唯一名额，需物理删除才能重建同 `system_code`。
...
| `dm_component` | 主键 `(tenant_id, project_id, system_code)`（自然键，无软删） |
...
内容表统一提供项目查询、负责人查询索引；`dm_plan` 提供维度和系统索引，`dm_report` 提供周期索引，`dm_topic` 提供颗粒度和类型索引；问题、会议、目标表和字段按项目、状态、名称及关联目标提供组合索引。系统关联统一按 `(project_id, system_code)` 检索，不依赖 `arch_physical_subsystem.id`。文件内容不保存摘要，也不执行跨表重复查重。

### 6.3 软删除与彻底删除

- 内容、问题、会议和附件关系使用 `deleted` 软删除并记录删除人和时间；目标表和字段仅使用 `deleted` 标记，不记录删除人和时间；关系表 `dm_meeting_system`、`dm_issue_relation` 无软删除。
- `dm_component` 无 `deleted` 列，使用 `enabled` 启停（1 启用、0 停用）；停用后业务关联仍可保留编号，但活动清单排除；物理删除后同 `system_code` 可重建。
...
3. 关联目标（会议、目标表、字段、系统/组件）必须同时满足租户、项目和未删除/启用条件；关联系统按当前项目 `dm_component` 启用行校验。
...
- `dm_component` 采用自然键主键 `(tenant_id, project_id, system_code)` + `enabled` 启停：无 `id` 代理键、无 `deleted` 软删，停用即从活动清单排除但保留编号与历史关联，物理删除释放唯一名额。
...
- 组件自然键主键：`dm_component` 主键为 `(tenant_id, project_id, system_code)`，无 `id` 代理键、无 `deleted` 软删，使用 `enabled` 启停；内容表关联字段由 `component_id` 收敛为 `system_code`，看板快照维度由 `component_id` 收敛为 `system_code`。
- 专题域字段：`dm_topic` 增加 `granularity`/`topic_type_code`/`topic_summary` 域字段，支持专题颗粒度与类型分类。
...
- 运行关系：`server/src/modules/data-migration` 的 Controller/Service SQL、`ContentAssetTables` 登记常量、`ContentDocCodeGenerator` 编号生成器、前端路由和菜单权限种子。
