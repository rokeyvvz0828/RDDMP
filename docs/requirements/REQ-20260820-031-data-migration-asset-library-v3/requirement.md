...
> 2026-09-06 经用户确认：将「数迁资产内容 › 迁移检核规则」从通用结构化资产薄页升级为专属域功能，作为本需求增量并入现有分支 `feat/REQ-20260820-031-data-migration`。用户同时确认：`doc_code`/`doc_name` 不使用并删除，缺少字段重建；单条编辑只编辑一条数据；参数类别命名按推荐 `DM_RULE_TARGET_TYPE`/`DM_RULE_CATEGORY`；检核规则与目标表/中间表**不做关联联动校验**（表/字段名自由文本）；批量上传选择文件不自动上传，点击“提交导入”后才上传解析。2026-09-06 追加：批量导入改为**按文件逐行填写**，检核目标类型、检核规则大类、系统编号等维度随每行填写，导入前不再要求提前选定这些维度；项目归属沿用 T31-r1，由顶部全局项目上下文静默提供。
...
- 检核目标类型：`DM_RULE_TARGET_TYPE`（迁出/迁入中间表/迁入目标表），由“系统管理/参数管理”维护；批量导入时按每行填写展示名并映射为码值。
- 检核规则大类：`DM_RULE_CATEGORY`（基础/表间横向/纵向/总分核对），由“系统管理/参数管理”维护；批量导入时按每行填写展示名并映射为码值。
- 关联系统：列表/新增/编辑下拉选择当前项目 `dm_component` 启用清单，`(project_id, system_code)` 合法且启用；批量导入时每行填写 `系统编号`，服务端逐行校验归属当前项目。
- 表英文名、表中文名、字段英文名称、字段中文名称：可空自由文本，不做目标表/中间表关联校验。
- 规则编码：必填且 `(tenant_id, rule_code)` 全局唯一，≤96 字符。
- 规则编码说明、检核规则说明：可空自由文本。
- 下载导出列与列表一致：目标类型、规则大类、系统编号/名称、表/字段英中名、规则编码/说明、检核说明；页面和导出均不回显项目字段。
- 必填：检核目标类型、检核规则大类、关联系统、规则编码；项目归属由服务端按请求中的全局项目上下文校验，页面不提供项目字段。
...
1. 列表可按检核目标类型、检核规则大类、关联系统、规则关键字、表/字段关键字组合筛选并分页，展示目标类型、规则大类、系统编号/名称、表/字段英中名、规则编码/说明、检核说明，不展示项目字段。
2. 新增/编辑必填项由服务端校验；规则编码全局唯一（编辑排除自身）；参数停用/未知编码、项目外系统、越权编辑/删除均被拒绝。
3. 批量上传直接使用顶部全局项目上下文，检核目标类型/检核规则大类/系统编号随文件逐行填写（模板 10 列）；选择文件后不自动上传，点击“提交导入”才上传解析；逐行校验失败行跳过并提示，不影响其余行入库。
...
## 增量：前端项目回显与上传样式治理（T47）

> 2026-09-07 用户确认按计划执行数据迁移前端规范整改。REQ-061 属于投产模块，其普通 YAML 格式会阻断仓库全量治理，但不属于本增量可写范围；本增量不得修改或绕过该文件，只在验收证据中将其记录为外部扰动。

### 目标与范围

- 重新执行 T31-r1 模块级不变量：数据迁移页面、列表、详情、弹层和确认文案不得展示「所属项目」「项目名称」或动态项目名称；`projectId` 仍由顶部全局项目上下文提供并参与请求。
- Excel 导入和主要源文件采用全宽拖拽区；附件追加或替换采用按钮入口与统一文件列表。两种形态共享语义主题、间距、文件名收缩和移动端约束。
- 自定义文件列表场景关闭 Element Plus 默认文件列表，避免重复文件名和重复移除操作。
- 上传相关业务样式集中到 `web/src/modules/data-migration/data-migration.css`，页面不再重复定义附件列表样式或使用内联十六进制颜色。
- 不修改后端、数据库、上传 API、公共 UI 组件、全局项目切换器、权限或审计。

### 验收标准

1. 当前数据迁移运行时 UI 无项目字段或动态项目名称；目标表 Excel 模板的「所属项目编码」列说明作为后端文件契约保留。
2. 所有拖拽上传根节点与 `.el-upload-dragger` 占满父容器，在 `1280x800`、`375x812`、`390x844`、`430x932` 无页面级横向溢出。
3. 附件列表使用模块共享样式，待上传、已绑定、失败和操作状态使用语义主题变量。
4. `ReportsPage` 单文件、批量和编辑重传不同时渲染 Element Plus 默认列表与自定义预览列表。
5. `npm --prefix web run build`、REQ-031 定向范围检查与 `git diff --check` 通过；真实浏览器覆盖项目字段、上传入口、文件列表和横向溢出。
6. `node scripts/check-all-governance.mjs` 若仍仅因 REQ-061 失败，则如实记录为范围外阻断，不将其描述为本增量通过，也不阻断数据迁移专项结论。

## 增量：整体看板实时指标与滚动下钻（2026-09-08）

> 2026-09-08 用户批准：整体看板采用已确认的高保真响应式排版；取消前端轮询、后端定时任务和快照读写；13 个指标分别实时请求并独立反馈；下钻仅在点击后按滚动分页加载。

### 功能目标与统计口径

- 整体看板展示 13 个当前项目实时指标：迁移整体方案、组件级迁移方案、项目级专题、组件级专题、汇报资料、会议纪要、问题清单、投产及演练资料、迁移映射资料、迁移检核规则、迁移参数、迁移过程依赖文件、迁移程序。
- 迁移整体方案统计 `dm_plan.granularity='PROJECT'`；组件级迁移方案统计 `dm_plan.granularity='SYSTEM'`。
- 项目级专题统计 `dm_topic.granularity='PROJECT'`；组件级专题统计 `dm_topic.granularity='SYSTEM'`。
- 其余九项分别统计 `dm_report`、`dm_meeting`、`dm_issue`、`dm_release_drill`、`dm_mapping_doc`、`dm_rule`、`dm_parameter`、`dm_dependency`、`dm_script`。
- 所有统计由服务端认证主体限定 `tenant_id`，请求必须携带当前全局项目解析出的 `projectId`，并只统计 `deleted=0` 的有效记录。

### 实时请求与快照退役

- 指标统一通过白名单路径按指标码独立请求；单个请求只返回一个指标的数量与计算时间。
- 前端最多并发 4 个指标请求。每张卡独立显示加载、成功、失败和单项重试；单项失败不清空其他指标。
- 首次进入、切换项目、手动刷新或单项重试时才查询；不实现前端轮询，不保留后端定时刷新任务。
- `dm_dashboard_snapshot` 表本次保留以降低回滚成本，但看板不再读取或写入该表；`DashboardSnapshotScheduler` 不再注册执行。
- 总量仅在 13 项全部成功后显示。存在失败项时显示未完成状态，不将部分和作为资产总量。

### 下钻与页面行为

- 点击指标后打开看板右侧下钻抽屉，首次打开才请求第一页；不随看板预取任何明细。
- 下钻每页 20 条，滚动接近底部时加载下一页；同一抽屉同时只允许一个翻页请求。
- 指标计数和下钻明细共享同一指标定义与项目/租户/有效状态条件，保证数量与明细口径一致。
- 项目切换时取消或废弃旧项目请求结果，关闭抽屉并清空旧数据；分页失败保留已加载记录并允许继续重试。
- 桌面端采用总量、四项方案/专题指标、九项内容资产和构成图的分层布局；平板重排总量与四项核心指标；手机端两列指标、图表下移、抽屉近全宽且页面无横向溢出。

### 验收标准

1. 13 个指标均由独立实时请求返回，卡片加载和失败状态互不阻塞，前端同时在途请求不超过 4 个。
2. 不存在前端轮询；后端快照定时任务不再运行；整体看板接口不读写 `dm_dashboard_snapshot`。
3. 每个指标数量与对应下钻完整分页结果在同一项目、租户和有效状态条件下可交叉核对。
4. 下钻首屏按点击加载，滚动分页每次 20 条，不预取未打开指标；空、首屏失败、后续页失败和全部加载完成状态可辨认并可恢复。
5. 缺少项目、不可访问项目、未知指标码和无看板权限的请求分别失败关闭，旧项目响应不能覆盖新项目页面。
6. 单指标查询 P95 不高于 500ms，13 项全部完成不高于 2s，下钻分页 P95 不高于 800ms；`EXPLAIN ANALYZE` 证明查询受项目范围索引约束。
7. `mvn -pl :ccb-data-migration -am test`、`npm --prefix web run build`、治理与范围检查通过；真实浏览器在 `1280x800`、`375x812`、`390x844`、`430x932` 验收独立 loading、部分失败、单项重试、项目切换、滚动分页、明暗主题和横向溢出。
