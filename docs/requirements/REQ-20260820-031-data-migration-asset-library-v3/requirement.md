...
> 2026-09-06 经用户确认：将「数迁资产内容 › 迁移检核规则」从通用结构化资产薄页升级为专属域功能，作为本需求增量并入现有分支 `feat/REQ-20260820-031-data-migration`。用户同时确认：`doc_code`/`doc_name` 不使用并删除，缺少字段重建；单条编辑只编辑一条数据；参数类别命名按推荐 `DM_RULE_TARGET_TYPE`/`DM_RULE_CATEGORY`；检核规则与目标表/中间表**不做关联联动校验**（表/字段名自由文本）；批量上传选择文件不自动上传，点击“提交导入”后才上传解析。2026-09-06 追加：批量导入改为**按文件逐行填写**，检核目标类型、检核规则大类、系统编号等维度随每行填写，导入前不再要求提前选定这些维度（仅需选定所属项目）。
...
3. 批量上传前置仅需选择所属项目，检核目标类型/检核规则大类/系统编号随文件逐行填写（模板 10 列）；选择文件后不自动上传，点击“提交导入”才上传解析；逐行校验失败行跳过并提示，不影响其余行入库。
