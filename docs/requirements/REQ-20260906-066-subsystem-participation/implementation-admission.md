...
### 退出守卫兼容实施范围
依据本次Owner审批，将ProjectService及现有ProjectWorkflowMemberGuardTest纳入精确路径。仅将单一ProjectMemberRemovalGuard注入改为集合聚合，接口和工作流守卫保持不变；架构服务实现既有公开接口，平台不依赖架构模块，不访问其他模块表。回归覆盖多守卫、停用、删除及租户项目隔离。无新增数据库迁移或前端界面。

## 2026-09-07 并发资格纠偏（plan-7）
在用户已确认的Owner审批范围内补充项目资格并发控制，精确路径已登记scope/control-plan。公开接口和模块依赖不变，业务模块不读取平台私有数据；仅平台内部JdbcProjectMemberReferenceQuery在实际事务中对当前项目有效成员及关联用户使用共享锁，非事务列表保持无锁。ProjectService停用/退出在运行已有责任守卫前锁定目标成员，保证新分派与退出不能同时通过旧快照。锁保持至调用方事务结束，不新增全局锁或数据表。
系统模块测试新增复用项目现有版本的Testcontainers测试依赖，覆盖读取先提交/撤销先提交/旧快照三种时序；既有聚合守卫和全仓测试均需复验。潜在代价是同项目资格写入等待，数据库死锁应回滚而非放行；上线仍需人工权限专项复核。

## 2026-09-07 关联测试范围补充授权
用户在明确列出 ArchitectureOptionsControllerTest.java 缺少可写范围后回复“可以”，授权仅增加该测试路径并同步 physicalUsers / physicalParameters 的两条精确断言；其他选项限制不变，不扩大生产权限。
