...
1. 已部署包含测试管理模块的应用，并已成功执行 V137 至 V144 Flyway 迁移。
2. `pm_project` 中存在编码为 `RDDMP-PLATFORM` 的项目；如测试环境编码不同，先修改 SQL 文件开头的 `@project_code`。
...
FROM tm_test_scope WHERE project_id = (SELECT id FROM pm_project WHERE project_code = 'RDDMP-PLATFORM' LIMIT 1)
