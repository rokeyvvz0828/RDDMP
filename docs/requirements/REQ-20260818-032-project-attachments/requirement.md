...
- 附件分类按项目维护，允许新增自定义分类；上传时可选择分类，未指定分类的附件归入“未分类”，已有附件可重新选择分类。
...
- `GET /api/project/{projectId}/attachment-categories`：返回当前项目自定义附件分类。
- `POST /api/project/{projectId}/attachment-categories`：创建项目附件分类，JSON 字段 `name`。
- `POST /api/project/{projectId}/attachments`：multipart 字段 `file` 和可选 `categoryId`，上传并绑定项目。
- `PUT /api/project/{projectId}/attachments/{attachmentId}/category`：修改附件分类，JSON 字段 `categoryId`，传空值归入“未分类”。
...
- 数据库已由 `V61__persistent_project_attachments.sql` 建立附件表，本次追加 `V82__project_attachment_categories.sql`，增加项目分类表和附件分类引用字段。
...
7. 项目更新权限成员可以新建分类、按分类上传并修改已有附件分类；历史或未指定分类附件显示为“未分类”。
...
- 回退或补偿：回退应用代码后保留 V61/V82 数据；不执行生产 DROP，必要时关闭附件入口并保留已绑定附件及分类只读。
