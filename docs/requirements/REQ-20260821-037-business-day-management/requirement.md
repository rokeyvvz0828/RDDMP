...
- Sidebar 在“测试管理”下只保留一个二级“营业日管理”入口，其图标和文字与“应用组装测试”等二级菜单严格对齐；日历概览、日历安排、跑批需求、测试环境管理改为页面顶部导航，并支持 URL 深链接。
...
1. Sidebar 只显示一个“营业日管理”二级入口，其图标和文字横坐标与其他二级菜单一致；进入后顶部显示四个导航项，刷新和复制 URL 后仍保持当前视图。
