-- Flowable 7.0.1 MySQL schema assembled from official module scripts. All ACT_* tables are engine-owned.

-- Pre-create every module table because official module scripts add cross-module indexes and constraints.
...
-- Apply official indexes, properties and constraints after all tables exist.
...
-- CCB business extensions. Flowable ACT_* tables above are managed by the engine.
ALTER TABLE wf_definition
    ADD COLUMN model_schema_version INT NOT NULL DEFAULT 1,
    ADD COLUMN bpmn_xml LONGTEXT NULL,
    ADD COLUMN deployment_id VARCHAR(64) NULL,
    ADD COLUMN node_mapping_json JSON NULL,
    ADD KEY idx_wf_definition_deployment (tenant_id, deployment_id);
...
ALTER TABLE wf_instance
    ADD COLUMN flowable_process_instance_id VARCHAR(64) NULL,
    ADD COLUMN flowable_process_definition_id VARCHAR(64) NULL,
    ADD COLUMN variables_json JSON NULL,
    ADD KEY idx_wf_instance_flowable (tenant_id, flowable_process_instance_id);

ALTER TABLE wf_task
    ADD COLUMN flowable_task_id VARCHAR(64) NULL,
    ADD COLUMN action_key VARCHAR(64) NULL,
    ADD KEY idx_wf_task_flowable (tenant_id, flowable_task_id);
