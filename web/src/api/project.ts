...
import type { Project, ProjectMember, ProjectOptions, ProjectPlan, ProjectPlanGroup, ProjectPlanGroupPayload, ProjectRisk, ProjectRiskComment, ProjectRole, ProjectUserOption, ProjectOrganization, ProjectStage, ProjectReleaseCalendar, ProjectAnnouncement, ProjectAnnouncementCategory, ProjectReleaseCalendarToneKey } from '../types/project'
...
export function createProjectReleaseCalendar(id: number, payload: { title: string; release_start_date: string; release_end_date: string; remark?: string; theme_key: ProjectReleaseCalendarToneKey }) { return http.post<ApiResponse<ProjectReleaseCalendar>>(`/project/${id}/release-calendar`, payload, withProjectContext(id)) }
export function updateProjectReleaseCalendar(id: number, calendarId: number, payload: { title: string; release_start_date: string; release_end_date: string; remark?: string; theme_key: ProjectReleaseCalendarToneKey; row_version: number }) { return http.put<ApiResponse<ProjectReleaseCalendar>>(`/project/${id}/release-calendar/${calendarId}`, payload, withProjectContext(id)) }
...
export function getProjectAnnouncements(id: number, category?: ProjectAnnouncementCategory) { return http.get<ApiResponse<ProjectAnnouncement[]>>(`/project/${id}/announcements`, { ...withProjectContext(id), params: category ? { stageCode: category } : undefined }) }
