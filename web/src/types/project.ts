...
export const projectCreationTypeLabels: Record<ProjectCreationType, string> = { NEW: '新建', CONTINUATION: '续建' }
...
export interface ProjectStage { id: number; project_id: number; stage_code: string; phase: string; stage_name: string; phase_name?: string | null; sort_no: number; status: number; has_master_plans: boolean | number; locked?: boolean; locked_reason?: string | null; created_at?: string; updated_at?: string }
...
export interface Project { id: number; project_code: string; project_name: string; description?: string | null; status: ProjectStatus; creation_type?: ProjectCreationType; plan_number_rule?: string | null; child_plan_number_rule?: string | null; risk_number_rule?: string | null; owner_id: number; owner_name?: string | null; planned_start_date?: string | null; planned_end_date?: string | null; actual_end_date?: string | null; member_count: number; plan_count: number; completed_plan_count: number; plan_progress: number; plan_stages?: ProjectStage[]; plans?: ProjectPlan[]; plan_groups?: ProjectPlanGroup[]; risks?: ProjectRisk[]; members?: ProjectMember[]; roles?: ProjectRole[]; project_organizations?: ProjectOrganization[]; attachments?: ProjectAttachment[]; created_at?: string; updated_at?: string }
