...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Briefcase, Calendar, ChatDotRound, Delete, Document, Download, Edit, Folder, FullScreen, Lock, Plus, ScaleToOriginal, Search, Setting, Upload, User, View } from '@element-plus/icons-vue'
import type { UploadFile } from 'element-plus'
import { apiErrorMessage } from '../api/error'
import { createProjectAnnouncement, deleteProjectAnnouncement, getCurrentProjectAnnouncements, getProjectAnnouncements, updateProjectAnnouncement } from '../api/project'
import { projectAnnouncementCategories } from '../types/project'
import type { ProjectAnnouncement, ProjectAnnouncementCategory } from '../types/project'
...
import { createProject, createProjectMember, createProjectOrganization, createProjectPlan, createProjectPlanGroup, createProjectReleaseCalendar, createProjectRisk, createProjectRiskComment, createProjectRole, createProjectStage, deleteProject, deleteProjectMember, deleteProjectOrganization, deleteProjectPlan, deleteProjectPlanGroup, deleteProjectReleaseCalendar, deleteProjectRisk, deleteProjectRole, deleteProjectStage, getProject, getProjectOptions, getProjectReleaseCalendar, getProjectRiskComments, getProjectRolePermissions, getProjectUserOptions, getProjectWorkbench, moveProjectPlanToGroup, saveProjectRolePermissions, updateProject, updateProjectMember, updateProjectOrganization, updateProjectPlan, updateProjectReleaseCalendar, updateProjectRisk, updateProjectRole, updateProjectSettings, updateProjectStage } from '../api/project'
import type { Project, ProjectMember, ProjectOptions, ProjectOrganization, ProjectPlan, ProjectRisk, ProjectRiskComment, ProjectRole, ProjectStage, ProjectStatus, PlanStatus, ProjectUserOption, ProjectOrganizationOption, ProjectPlanGroupColorToken, ProjectReleaseCalendar } from '../types/project'
import type { ProjectReleaseCalendarToneKey } from '../types/project'
...
const projectTabs = ['overview', 'plans', 'risk', 'organization', 'release-calendar', 'announcements', 'settings', 'attachments'] as const
...
const releaseCalendarEntries = ref<ProjectReleaseCalendar[]>([])
const releaseCalendarLoading = ref(false)
const releaseCalendarError = ref('')
const releaseCalendarMonth = ref(new Date().toISOString().slice(0, 7))
const releaseCalendarYear = new Date().getFullYear()
const releaseCalendarDialog = ref(false)
const releaseCalendarSubmitting = ref(false)
const releaseCalendarEditingId = ref<number | null>(null)
const releaseCalendarForm = reactive<{ title: string; release_date_range: string[]; remark: string; theme_key: ProjectReleaseCalendarToneKey; row_version: number }>({ title: '', release_date_range: [], remark: '', theme_key: 'tone-1', row_version: 0 })
const releaseCalendarThemeOptions: Array<{ value: ProjectReleaseCalendarToneKey; label: string }> = [
  { value: 'tone-1', label: '颜色一' }, { value: 'tone-2', label: '颜色二' }, { value: 'tone-3', label: '颜色三' }, { value: 'tone-4', label: '颜色四' }, { value: 'tone-5', label: '颜色五' }
]
const announcements = ref<ProjectAnnouncement[]>([])
const currentAnnouncements = ref<ProjectAnnouncement[]>([])
const announcementCategory = ref<ProjectAnnouncementCategory>('PROJECT')
const announcementsLoading = ref(false)
const announcementsError = ref('')
const announcementDialog = ref(false)
const announcementDetailDialog = ref(false)
const announcementDetail = ref<ProjectAnnouncement | null>(null)
const announcementSaving = ref(false)
const announcementEditingId = ref<number | null>(null)
const announcementForm = reactive({ stage_code: '', title: '', content_html: '', pinned: false, row_version: 0 })
const announcementEditor = shallowRef<any>()
const announcementEditorHtml = ref('')
const releaseCalendarWeekdays = ['日', '一', '二', '三', '四', '五', '六']
const releaseCalendarCanGoPrevious = computed(() => Number(releaseCalendarMonth.value.slice(0, 4)) === releaseCalendarYear && Number(releaseCalendarMonth.value.slice(5, 7)) > 1)
const releaseCalendarCanGoNext = computed(() => Number(releaseCalendarMonth.value.slice(0, 4)) === releaseCalendarYear && Number(releaseCalendarMonth.value.slice(5, 7)) < 12)
const releaseCalendarDays = computed(() => { const [year, month] = releaseCalendarMonth.value.split('-').map(Number); const count = new Date(year, month, 0).getDate(); const leading = new Date(year, month - 1, 1).getDay(); return [...Array<string | null>(leading).fill(null), ...Array.from({ length: count }, (_, index) => `${releaseCalendarMonth.value}-${String(index + 1).padStart(2, '0')}`)] })
const releaseCalendarMobileGroups = computed(() => {
  const groups = new Map<string, ProjectReleaseCalendar[]>()
  releaseCalendarDays.value.filter((date): date is string => Boolean(date)).forEach(date => {
    const entries = calendarEntriesFor(date)
    if (entries.length) groups.set(date, entries)
  })
...
async function refreshProject(projectId: number) { detailLoading.value = true; try { if (selectedProject.value?.id !== projectId) newlyCreatedStageRowIds.value = new Set(); selectedProject.value = (await getProject(projectId)).data.data; ensureProjectUserOptions(selectedProject.value); resetSettingsForm(); if (activeTab.value === 'attachments') { resetAttachmentQuery(); await Promise.all([loadProjectAttachments(), loadProjectAttachmentCategories()]) } if (activeTab.value === 'release-calendar' || activeTab.value === 'overview') await loadReleaseCalendar() } catch (error) { newlyCreatedStageRowIds.value = new Set(); selectedProject.value = null; ElMessage.error(apiErrorMessage(error, '项目详情加载失败')); await router.replace({ name: 'projects', query: {} }) } finally { detailLoading.value = false } }
...
async function loadReleaseCalendar() { if (!selectedProject.value) return; releaseCalendarLoading.value = true; releaseCalendarError.value = ''; try { releaseCalendarEntries.value = (await getProjectReleaseCalendar(selectedProject.value.id, releaseCalendarMonth.value)).data.data } catch (error) { releaseCalendarEntries.value = []; releaseCalendarError.value = apiErrorMessage(error, '投产日历加载失败') } finally { releaseCalendarLoading.value = false } }
function changeReleaseCalendarMonth(offset: number) { const next = new Date(`${releaseCalendarMonth.value}-01T00:00:00`); next.setMonth(next.getMonth() + offset); if (next.getFullYear() === releaseCalendarYear) releaseCalendarMonth.value = `${next.getFullYear()}-${String(next.getMonth() + 1).padStart(2, '0')}` }
function calendarEntriesFor(date: string) { return releaseCalendarEntries.value.filter(item => String(item.release_start_date).slice(0, 10) <= date && String(item.release_end_date).slice(0, 10) >= date) }
function releaseCalendarRangeLabel(entry: ProjectReleaseCalendar) { const start = formatDateOnly(String(entry.release_start_date).slice(0, 10)); const end = formatDateOnly(String(entry.release_end_date).slice(0, 10)); return start === end ? start : `${start} 至 ${end}` }
function releaseCalendarThemeKeyForEdit(themeKey?: ProjectReleaseCalendar['theme_key']): ProjectReleaseCalendarToneKey { return releaseCalendarThemeOptions.find(option => option.value === themeKey)?.value || 'tone-1' }
function releaseCalendarThemeClass(themeKey?: ProjectReleaseCalendar['theme_key']) { return `theme-${releaseCalendarThemeKeyForEdit(themeKey)}` }
function resetReleaseCalendarForm() { Object.assign(releaseCalendarForm, { title: '', release_date_range: [], remark: '', theme_key: 'tone-1', row_version: 0 }); releaseCalendarEditingId.value = null }
function openReleaseCalendarDialog(entry?: ProjectReleaseCalendar) { if (entry) { releaseCalendarEditingId.value = entry.id || null; Object.assign(releaseCalendarForm, { title: entry.title, release_date_range: [String(entry.release_start_date).slice(0, 10), String(entry.release_end_date).slice(0, 10)], remark: entry.remark || '', theme_key: releaseCalendarThemeKeyForEdit(entry.theme_key), row_version: entry.row_version }) } else resetReleaseCalendarForm(); releaseCalendarDialog.value = true }
function openReleaseCalendarForDate(date: string) { openReleaseCalendarDialog({ id: 0, project_id: selectedProject.value?.id || 0, title: '', release_start_date: date, release_end_date: date, row_version: 0 }) }
async function saveReleaseCalendar() { const [startDate, endDate] = releaseCalendarForm.release_date_range; if (!selectedProject.value || !releaseCalendarForm.title.trim() || !startDate || !endDate) { ElMessage.warning('请填写投产标题和日期范围'); return } if (startDate > endDate) { ElMessage.warning('投产开始日期不能晚于结束日期'); return } releaseCalendarSubmitting.value = true; try { const payload = { title: releaseCalendarForm.title.trim(), release_start_date: startDate, release_end_date: endDate, remark: releaseCalendarForm.remark.trim() || undefined, theme_key: releaseCalendarForm.theme_key }; if (releaseCalendarEditingId.value) await updateProjectReleaseCalendar(selectedProject.value.id, releaseCalendarEditingId.value, { ...payload, row_version: releaseCalendarForm.row_version }); else await createProjectReleaseCalendar(selectedProject.value.id, payload); releaseCalendarDialog.value = false; await loadReleaseCalendar(); ElMessage.success('投产日历已保存') } catch (error) { ElMessage.error(apiErrorMessage(error, '投产日历保存失败')) } finally { releaseCalendarSubmitting.value = false } }
async function removeReleaseCalendar(entry: ProjectReleaseCalendar) { if (!selectedProject.value) return; try { await ElMessageBox.confirm(`确认删除投产安排“${entry.title}”吗？删除后不可恢复。`, '删除投产安排', { type: 'warning' }); await deleteProjectReleaseCalendar(selectedProject.value.id, entry.id, entry.row_version); await loadReleaseCalendar(); ElMessage.success('投产安排已删除') } catch (error) { const action = messageBoxAction(error); if (action !== 'cancel' && action !== 'close') ElMessage.error(apiErrorMessage(error, '投产日历删除失败')) } }
async function loadAnnouncements(category: ProjectAnnouncementCategory = announcementCategory.value) { if (!selectedProject.value) return; announcementsLoading.value = true; announcementsError.value = ''; try { announcements.value = (await getProjectAnnouncements(selectedProject.value.id, category)).data.data; currentAnnouncements.value = (await getCurrentProjectAnnouncements(selectedProject.value.id)).data.data } catch (error) { announcements.value = []; currentAnnouncements.value = []; announcementsError.value = apiErrorMessage(error, '项目公告加载失败') } finally { announcementsLoading.value = false } }
function changeAnnouncementCategory(value: string | number) { const category = String(value) as ProjectAnnouncementCategory; if (projectAnnouncementCategories.some(item => item.value === category)) void loadAnnouncements(category) }
function announcementPlainText(html: string) { const node = document.createElement('div'); node.innerHTML = html; return (node.textContent || node.innerText || '').replace(/\s+/g, ' ').trim() }
function openAnnouncementDetail(item: ProjectAnnouncement) { announcementDetail.value = item; announcementDetailDialog.value = true }
function openAnnouncementDialog(item?: ProjectAnnouncement) { announcementEditingId.value = item?.id || null; Object.assign(announcementForm, { stage_code: item?.stage_code || announcementCategory.value, title: item?.title || '', content_html: item?.content_html || '', pinned: Boolean(item?.pinned), row_version: item?.row_version || 0 }); announcementEditorHtml.value = announcementForm.content_html; announcementDialog.value = true }
function announcementEditorCreated(editor: any) { announcementEditor.value = editor; editor.setHtml(announcementForm.content_html || '') }
function announcementEditorChanged(editor: any) { announcementEditorHtml.value = editor.getHtml(); announcementForm.content_html = announcementEditorHtml.value }
function announcementEditorClosed() { announcementEditor.value?.destroy(); announcementEditor.value = undefined }
async function saveAnnouncement() { if (!selectedProject.value || !announcementForm.stage_code || !announcementForm.title.trim() || !announcementPlainText(announcementForm.content_html)) { ElMessage.warning('请填写公告栏位、标题和正文'); return } announcementSaving.value = true; try { const payload = { stage_code: announcementForm.stage_code, title: announcementForm.title.trim(), content_html: announcementForm.content_html, pinned: announcementForm.pinned }; if (announcementEditingId.value) await updateProjectAnnouncement(selectedProject.value.id, announcementEditingId.value, { ...payload, row_version: announcementForm.row_version }); else await createProjectAnnouncement(selectedProject.value.id, payload); announcementDialog.value = false; await loadAnnouncements(); ElMessage.success('项目公告已保存') } catch (error) { ElMessage.error(apiErrorMessage(error, '项目公告保存失败')) } finally { announcementSaving.value = false } }
async function toggleAnnouncementPinned(item: ProjectAnnouncement) { if (!selectedProject.value) return; const nextPinned = !Boolean(item.pinned); try { await ElMessageBox.confirm(nextPinned ? `确认置顶公告“${item.title}”吗？` : `确认取消置顶公告“${item.title}”吗？`, nextPinned ? '置顶公告' : '取消置顶', { type: 'warning', confirmButtonText: nextPinned ? '置顶' : '取消置顶', cancelButtonText: '取消' }); await updateProjectAnnouncement(selectedProject.value.id, item.id, { stage_code: item.stage_code, title: item.title, content_html: item.content_html, pinned: nextPinned, row_version: item.row_version }); await loadAnnouncements(); ElMessage.success(nextPinned ? '公告已置顶' : '公告已取消置顶') } catch (error) { const action = messageBoxAction(error); if (action !== 'cancel' && action !== 'close') ElMessage.error(apiErrorMessage(error, nextPinned ? '公告置顶失败' : '取消置顶失败')) } }
async function removeAnnouncement(item: ProjectAnnouncement) { if (!selectedProject.value) return; try { await ElMessageBox.confirm(`确认删除公告“${item.title}”吗？删除后不可恢复。`, '删除公告', { type: 'warning' }); await deleteProjectAnnouncement(selectedProject.value.id, item.id, item.row_version); await loadAnnouncements(); ElMessage.success('项目公告已删除') } catch (error) { const action = messageBoxAction(error); if (action !== 'cancel' && action !== 'close') ElMessage.error(apiErrorMessage(error, '项目公告删除失败')) } }
...
async function setProjectTab(tab: string | number) { const normalizedTab = normalizeProjectTab(String(tab)); clearTabLoadingTimer(); tabContentLoading.value = true; if (route.name === 'project-detail' && route.params.projectId != null) { const query = new URLSearchParams(); Object.entries(route.query).forEach(([key, value]) => { const nextValue = Array.isArray(value) ? value[0] : value; if (key !== 'tab' && typeof nextValue === 'string') query.set(key, nextValue) }); query.set('tab', normalizedTab); window.history.replaceState(window.history.state, '', `${route.path}?${query.toString()}${route.hash}`) } if (normalizedTab === 'attachments') await Promise.all([loadProjectAttachments(), loadProjectAttachmentCategories()]); if (normalizedTab === 'release-calendar') await loadReleaseCalendar(); await nextTick(); await new Promise<void>(resolve => { tabLoadingTimer = window.setTimeout(resolve, 220) }); tabLoadingTimer = null; tabContentLoading.value = false }
...
async function saveProject() { if (saving.value) return; projectCreationTypeError.value = ['NEW', 'CONTINUATION'].includes(String(projectForm.creation_type)) ? '' : '请选择创建类型'; if (projectCreationTypeError.value) return; if (!String(projectForm.project_code || '').trim() || !String(projectForm.project_name || '').trim()) { ElMessage.warning('请填写项目编号和项目名称'); return }; if (!validDateRange(projectForm.planned_start_date, projectForm.planned_end_date) || !validDateRange(projectForm.planned_start_date, projectForm.actual_end_date)) { ElMessage.warning('项目结束日期必须大于等于开始日期'); return }; const creating = projectEditingId.value === null; saving.value = true; try { const response = creating ? await createProject(projectForm) : await updateProject(projectEditingId.value!, projectForm); selectedProject.value = response.data.data; projectContext.syncProject(response.data.data); if (creating) projectContext.select(response.data.data.project_code); ensureProjectUserOptions(selectedProject.value); resetSettingsForm(); projectDialog.value = false; await loadWorkbench(); await router.push({ name: 'project-detail', params: { projectId: String(selectedProject.value.id) } }); ElMessage.success(creating ? '项目已创建' : '项目已更新'); if (creating) window.location.reload() } catch (error) { ElMessage.error(apiErrorMessage(error, '项目保存失败')) } finally { saving.value = false } }
async function removeProject() { if (!selectedProject.value) return; try { await ElMessageBox.confirm('删除项目后，项目计划、成员和角色也将不再显示，确认继续吗？', '删除确认', { type: 'warning' }); const deletedProject = selectedProject.value; await deleteProject(deletedProject.id); projectContext.removeProject(deletedProject.project_code); selectedProject.value = null; await router.push({ name: 'projects' }); await loadWorkbench(); ElMessage.success('项目已删除') } catch (error) { const action = messageBoxAction(error); if (action !== 'cancel' && action !== 'close') ElMessage.error(apiErrorMessage(error, '项目删除失败')) } }
...
function projectCreationTypeTone(value?: string | null): 'primary' | 'warning' { return value === 'CONTINUATION' ? 'warning' : 'primary' }
...
watch(releaseCalendarMonth, () => { if (activeTab.value === 'release-calendar' || activeTab.value === 'overview') void loadReleaseCalendar() })
watch(activeTab, tab => { if (tab === 'announcements') void loadAnnouncements(); if (tab === 'overview' || tab === 'release-calendar') void loadReleaseCalendar() })
watch(() => selectedProject.value?.id, projectId => { if (projectId) void loadAnnouncements() })
...
<template>
  <section class="project-page">
    ...
    <template v-if="!selectedProject && !isProjectDetailRoute">
      ...
      <div class="project-card-grid">
        <el-card v-for="project in projects" :key="project.id" shadow="never" class="project-card" @click="openProject(project)">
          <div class="project-card__top"><span class="project-card__icon"><el-icon><Briefcase /></el-icon></span><span class="project-card__tags"><UiStatusTag :value="project.creation_type ?? 'NEW'" :labels="projectCreationTypeLabels" :tone="projectCreationTypeTone(project.creation_type)" /><UiStatusTag :value="project.status" :labels="projectStatusLabels" :tone="projectStatusTone(project.status)" /></span></div>
    ...
    <template v-else-if="selectedProject">
      ...
      <el-tabs v-model="activeTab" class="project-tabs" @tab-change="setProjectTab">
        <el-tab-pane name="overview"><template #label><el-icon><Briefcase /></el-icon><span>项目概览</span></template><div class="project-overview">
          ...
          <aside class="project-overview__announcements" v-loading="announcementsLoading"><div class="project-overview__announcements-heading"><div><span class="panel-kicker">PROJECT ANNOUNCEMENTS</span><h3>项目公告</h3></div><el-button text type="primary" @click="activeTab = 'announcements'">管理</el-button></div><el-alert v-if="announcementsError" type="error" :title="announcementsError" :closable="false" show-icon><template #default><el-button link type="primary" @click="loadAnnouncements">重试</el-button></template></el-alert><el-empty v-else-if="!currentAnnouncements.length" description="暂无项目公告" :image-size="44" /><button v-for="item in currentAnnouncements" v-else :key="item.id" type="button" class="project-overview__announcement" @click="openAnnouncementDetail(item)"><span><el-tag v-if="item.pinned" type="primary" size="small">置顶</el-tag><strong>{{ item.title }}</strong></span><time>{{ formatDateOnly(item.created_at) || '-' }}</time></button></aside>
          <section class="project-overview__release-calendar" v-loading="releaseCalendarLoading"><header><div><span class="panel-kicker">RELEASE CALENDAR</span><h3>投产日历</h3></div><div class="project-overview__calendar-actions"><el-button text circle title="上个月" :disabled="!releaseCalendarCanGoPrevious" @click="changeReleaseCalendarMonth(-1)"><el-icon><ArrowLeft /></el-icon></el-button><strong>{{ releaseCalendarMonth }}</strong><el-button text circle title="下个月" :disabled="!releaseCalendarCanGoNext" @click="changeReleaseCalendarMonth(1)"><el-icon><ArrowRight /></el-icon></el-button></div></header><el-alert v-if="releaseCalendarError" type="error" :title="releaseCalendarError" :closable="false" show-icon><template #default><el-button link type="primary" @click="loadReleaseCalendar">重试</el-button></template></el-alert><div v-else class="project-overview__calendar-grid"><span v-for="weekday in releaseCalendarWeekdays" :key="weekday">{{ weekday }}</span><template v-for="(date, index) in releaseCalendarDays" :key="date || `overview-blank-${index}`"><time v-if="date && calendarEntriesFor(date).length" :class="['has-release', releaseCalendarThemeClass(calendarEntriesFor(date)[0]?.theme_key)]" :aria-label="`${date} 投产安排：${calendarEntriesFor(date)[0]?.title || ''}`"><span class="project-overview__calendar-title">{{ calendarEntriesFor(date)[0]?.title }}</span><span class="project-overview__calendar-day">{{ Number(date.slice(-2)) }}</span></time><time v-else :class="{ 'is-empty': !date }">{{ date ? Number(date.slice(-2)) : '' }}</time></template></div></section>
      ...
      <el-tab-pane name="plans"><template #label><el-icon><Calendar /></el-icon><span>项目计划</span></template>
      ...
      <el-tab-pane name="risk" label="项目风险"><div class="project-tab-panel project-risk-panel"><div class="project-section-heading project-section-heading--actions"><el-button v-if="canCreateRisk && isOwner" type="primary" @click="openCreateRisk"><el-icon><Plus /></el-icon>新增风险</el-button></div><UiDataTable class="project-risks-table" :data="risks" row-key="id" border empty-text="暂无项目风险"><el-table-column prop="risk_code" label="编号" width="190" fixed="left" /><el-table-column prop="occurred_date" label="发生时间" width="120"><template #default="scope">{{ formatDateOnly(scope.row.occurred_date) || '-' }}</template></el-table-column><el-table-column label="项目阶段" min-width="130"><template #default="scope">{{ scope.row.project_phase_name || scope.row.project_phase || '-' }}</template></el-table-column><el-table-column label="紧急程度" width="110"><template #default="scope">{{ scope.row.urgency_name || scope.row.urgency || '-' }}</template></el-table-column><el-table-column label="当前状态" width="120"><template #default="scope"><UiStatusTag :value="scope.row.current_status" :labels="{ [scope.row.current_status]: scope.row.current_status_name || scope.row.current_status || '-' }" :tone="riskStatusTone(scope.row.current_status, scope.row.current_status_name)" /></template></el-table-column><el-table-column label="提出组织/组" min-width="160"><template #default="scope">{{ scope.row.proposer_org_name || '-' }}</template></el-table-column><el-table-column label="问题描述" min-width="260" show-overflow-tooltip><template #default="scope">{{ scope.row.problem_description || '-' }}</template></el-table-column><el-table-column label="关注等级" width="120"><template #default="scope">{{ scope.row.attention_level_name || scope.row.attention_level || '-' }}</template></el-table-column><el-table-column label="当前处理人" min-width="160"><template #default="scope">{{ scope.row.current_handler_name ? `${scope.row.current_handler_name}${scope.row.current_handler_phone ? ` / ${scope.row.current_handler_phone}` : ''}` : '-' }}</template></el-table-column><el-table-column label="操作" width="290" fixed="right"><template #default="scope"><el-button v-if="canUpdateRisk && isOwner" link type="primary" @click="openEditRisk(scope.row)"><el-icon><Edit /></el-icon>编辑</el-button><el-button link type="success" @click="openRiskProgress(scope.row)"><el-icon><ChatDotRound /></el-icon>进度描述</el-button><el-button v-if="canUpdateRisk && isOwner" link type="warning" @click="openRiskTracking(scope.row)"><el-icon><Edit /></el-icon>问题跟踪</el-button><el-button v-if="canDeleteRisk && isOwner" link type="danger" @click="removeRisk(scope.row)"><el-icon><Delete /></el-icon>删除</el-button></template></el-table-column></UiDataTable></div></el-tab-pane>
      <el-tab-pane name="organization"><template #label><el-icon><User /></el-icon><span>项目组织架构</span></template>
        <div class="project-tab-panel project-organization-panel">
          <div class="project-organization-layout">
            <aside class="project-organization-tree-panel">
              <div class="project-section-heading project-section-heading--actions"><el-button v-if="canCreateMember && isOwner" type="primary" circle title="新增根组织" @click="openCreateProjectOrganization()"><el-icon><Plus /></el-icon></el-button></div>
              <button type="button" class="project-organization-tree__all" :class="{ 'is-active': selectedProjectOrganizationId === null }" @click="selectedProjectOrganizationId = null"><el-icon><Folder /></el-icon><span>全部机构</span><strong>{{ members.length }}</strong></button>
              <el-tree v-if="projectOrganizationTreeOptions.length" class="project-organization-tree" :data="projectOrganizationTreeOptions" node-key="value" default-expand-all highlight-current @node-click="selectProjectOrganization"><template #default="{ data }"><div class="project-organization-tree__node"><span class="project-organization-tree__node-label" :title="data.label">{{ data.label }}</span><span v-if="isOwner && (canCreateMember || canUpdateMember || canDeleteMember)" class="project-organization-tree__node-actions"><el-button v-if="canCreateMember" text circle title="新增下级组织" @click.stop="openCreateProjectOrganization(Number(data.value))"><el-icon><Plus /></el-icon></el-button><el-button v-if="canUpdateMember" text circle title="编辑组织" @click.stop="projectOrganizationById(data.value) && openEditProjectOrganization(projectOrganizationById(data.value)!)"><el-icon><Edit /></el-icon></el-button><el-button v-if="canDeleteMember" text circle title="删除组织" @click.stop="projectOrganizationById(data.value) && removeProjectOrganization(projectOrganizationById(data.value)!)"><el-icon><Delete /></el-icon></el-button></span></div></template></el-tree>
              <el-empty v-else description="暂无项目组织，请先新增根组织" :image-size="56" />
            </aside>
            <div class="project-organization-maintenance">
              <section class="project-organization-section">
                <div class="project-section-heading project-section-heading--actions"><el-button v-if="canCreateMember && isOwner" type="primary" @click="openCreateMember"><el-icon><Plus /></el-icon>添加成员</el-button></div>
                <UiDataTable :data="filteredMembers" row-key="id" border empty-text="暂无项目成员"><el-table-column label="成员" min-width="220"><template #default="scope"><UiUserIdentity :user="{ id: scope.row.user_id, username: scope.row.username, displayName: scope.row.display_name, avatarUrl: scope.row.avatar_url }" :show-profile="false" /></template></el-table-column><el-table-column prop="org_name" label="所属机构" min-width="180"><template #default="scope">{{ scope.row.org_name || '未分配机构' }}</template></el-table-column><el-table-column label="项目角色" min-width="220"><template #default="scope"><el-tag v-for="role in scope.row.roles" :key="role.id" size="small" effect="plain" class="project-role-tag">{{ role.role_name }}</el-tag><span v-if="!scope.row.roles.length" class="muted">未分配角色</span></template></el-table-column><el-table-column label="状态" width="100"><template #default="scope"><UiStatusTag :value="scope.row.status" :labels="{ '0': '停用', '1': '有效' }" /></template></el-table-column><el-table-column v-if="canUpdateMember && isOwner || canDeleteMember && isOwner" label="操作" width="150" fixed="right"><template #default="scope"><el-button v-if="canUpdateMember && isOwner" link type="primary" @click="openEditMember(scope.row)"><el-icon><Edit /></el-icon>编辑</el-button><el-button v-if="canDeleteMember && isOwner" link type="danger" @click="removeMember(scope.row)"><el-icon><Delete /></el-icon>移除</el-button></template></el-table-column></UiDataTable>
              </section>
            </div>
          </div>
        </div>
      </el-tab-pane>
      <el-tab-pane name="release-calendar"><template #label><el-icon><Calendar /></el-icon><span>投产日历</span></template>
        <div class="project-tab-panel release-calendar" v-loading="releaseCalendarLoading">
          <div class="project-section-heading project-section-heading--actions"><div class="release-calendar__toolbar"><el-date-picker v-model="releaseCalendarMonth" type="month" value-format="YYYY-MM" :clearable="false" /><el-button type="primary" @click="openReleaseCalendarDialog()"><el-icon><Plus /></el-icon>新增投产安排</el-button></div></div>
          <el-alert v-if="releaseCalendarError" type="error" :title="releaseCalendarError" show-icon :closable="false"><template #default><el-button link type="primary" @click="loadReleaseCalendar">重试</el-button></template></el-alert>
           <template v-else><div class="release-calendar__desktop"><span v-for="weekday in releaseCalendarWeekdays" :key="weekday" class="release-calendar__weekday">周{{ weekday }}</span><article v-for="(date, index) in releaseCalendarDays" :key="date || `blank-${index}`" class="release-calendar__day" :class="{ 'is-blank': !date }"><template v-if="date"><div class="release-calendar__day-heading"><time>{{ Number(date.slice(-2)) }}</time><el-button v-if="!calendarEntriesFor(date).length" text circle type="primary" aria-label="新增该日期投产安排" title="新增该日期投产安排" @click="openReleaseCalendarForDate(date)"><el-icon><Plus /></el-icon></el-button></div><div v-for="entry in calendarEntriesFor(date)" :key="entry.id" class="release-calendar__entry-row"><button type="button" class="release-calendar__entry" @click="openReleaseCalendarDialog(entry)"><strong>{{ entry.title }}</strong><span>{{ releaseCalendarRangeLabel(entry) }}</span><small>{{ entry.remark || '暂无备注' }}</small></button><el-tooltip content="删除投产安排" placement="top"><el-button text circle type="danger" aria-label="删除投产安排" @click.stop="removeReleaseCalendar(entry)"><el-icon><Delete /></el-icon></el-button></el-tooltip></div></template></article></div>
          <div class="release-calendar__mobile"><el-empty v-if="!releaseCalendarEntries.length" description="本月暂无投产安排" /><section v-for="group in releaseCalendarMobileGroups" :key="group.date" class="release-calendar__mobile-group"><time>{{ formatDateOnly(group.date) }}</time><article v-for="entry in group.entries" :key="entry.id" class="release-calendar__mobile-item"><div><strong>{{ entry.title }}</strong><p>{{ releaseCalendarRangeLabel(entry) }}</p><p v-if="entry.remark">{{ entry.remark }}</p></div><span><el-button text type="primary" @click="openReleaseCalendarDialog(entry)">编辑</el-button><el-button text type="danger" @click="removeReleaseCalendar(entry)">删除</el-button></span></article></section></div></template>
        </div>
      </el-tab-pane>
        <el-tab-pane name="announcements"><template #label><el-icon><Document /></el-icon><span>公告管理</span></template>
          <div class="project-tab-panel project-announcements">
            <div class="project-section-heading project-section-heading--actions"><el-button type="primary" @click="openAnnouncementDialog()"><el-icon><Plus /></el-icon>发布{{ projectAnnouncementCategories.find(item => item.value === announcementCategory)?.label || '公告' }}</el-button></div>
            <el-tabs v-model="announcementCategory" class="project-announcement-tabs" @tab-change="changeAnnouncementCategory">
              <el-tab-pane v-for="category in projectAnnouncementCategories" :key="category.value" :name="category.value" :label="category.label" />
            </el-tabs>
            <div class="project-announcement-content" v-loading="announcementsLoading">
              <el-alert v-if="announcementsError" type="error" :title="announcementsError" show-icon :closable="false"><template #default><el-button link type="primary" @click="loadAnnouncements()">重试</el-button></template></el-alert>
              <template v-else><el-empty v-if="!announcements.length" :description="`${projectAnnouncementCategories.find(item => item.value === announcementCategory)?.label || '当前栏位'}暂无公告`" /><div v-else class="project-announcement-list"><article v-for="item in announcements" :key="item.id"><header><div><el-tag v-if="item.pinned" type="primary" size="small">置顶</el-tag><strong>{{ item.title }}</strong></div><div class="project-announcement-actions"><el-button text type="warning" @click="toggleAnnouncementPinned(item)">{{ item.pinned ? '取消置顶' : '置顶' }}</el-button><el-button text type="primary" @click="openAnnouncementDialog(item)">编辑</el-button><el-button text type="danger" @click="removeAnnouncement(item)">删除</el-button></div></header><p>{{ announcementPlainText(item.content_html) }}</p><small>{{ item.creator_name || '项目成员' }} · {{ formatDateOnly(item.created_at) || '-' }}</small></article></div></template>
            </div>
          </div>
        </el-tab-pane>
        <el-tab-pane name="settings"><template #label><el-icon><Setting /></el-icon><span>项目设置</span></template>
          <div class="project-tab-panel project-settings">
        ...
        <el-tab-pane name="attachments"><template #label><el-icon><Folder /></el-icon><span>项目附件</span></template>
          <div class="project-tab-panel project-attachments-panel">
            <div class="project-section-heading project-section-heading--actions">
      ...
      <div v-if="tabContentLoading" class="project-loading-overlay project-loading-overlay--tab-content" role="status" aria-live="polite">
        <div class="project-loading-panel">
          <span class="project-loading-spinner" aria-hidden="true" />
          <span>正在加载页签内容</span>
        </div>
    ...
    <el-dialog v-model="releaseCalendarDialog" :title="releaseCalendarEditingId ? '编辑投产安排' : '新增投产安排'" width="520px" destroy-on-close><el-form label-position="top" @submit.prevent="saveReleaseCalendar"><el-form-item label="颜色" required><div class="release-calendar-theme-picker" role="radiogroup" aria-label="选择颜色"><button v-for="option in releaseCalendarThemeOptions" :key="option.value" type="button" :class="['release-calendar-theme-swatch', option.value, { 'is-selected': releaseCalendarForm.theme_key === option.value }]" :aria-label="option.label" :aria-checked="releaseCalendarForm.theme_key === option.value" role="radio" @click="releaseCalendarForm.theme_key = option.value"><span aria-hidden="true">✓</span></button></div></el-form-item><el-form-item label="投产日期范围" required><el-date-picker v-model="releaseCalendarForm.release_date_range" type="daterange" value-format="YYYY-MM-DD" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" unlink-panels style="width:100%" /></el-form-item><el-form-item label="投产标题" required><el-input v-model="releaseCalendarForm.title" maxlength="128" show-word-limit /></el-form-item><el-form-item label="备注"><el-input v-model="releaseCalendarForm.remark" type="textarea" :rows="4" maxlength="1000" show-word-limit /></el-form-item></el-form><template #footer><el-button :disabled="releaseCalendarSubmitting" @click="releaseCalendarDialog = false">取消</el-button><el-button type="primary" :loading="releaseCalendarSubmitting" @click="saveReleaseCalendar">保存</el-button></template></el-dialog>
    ...
    <el-dialog v-model="announcementDialog" :title="announcementEditingId ? '编辑公告' : '发布公告'" width="min(760px, calc(100vw - 24px))" destroy-on-close @closed="announcementEditorClosed"><el-form label-position="top"><el-form-item label="公告栏位" required><el-tag type="info">{{ projectAnnouncementCategories.find(item => item.value === announcementForm.stage_code)?.label || announcementForm.stage_code }}</el-tag></el-form-item><el-form-item label="公告标题" required><el-input v-model="announcementForm.title" maxlength="128" show-word-limit /></el-form-item><el-form-item label="公告正文" required><div class="project-announcement-editor"><Toolbar :editor="announcementEditor" mode="default" /><WangEditor v-model="announcementEditorHtml" mode="default" @on-created="announcementEditorCreated" @on-change="announcementEditorChanged" /></div></el-form-item><el-form-item><el-checkbox v-model="announcementForm.pinned">置顶公告</el-checkbox></el-form-item></el-form><template #footer><el-button :disabled="announcementSaving" @click="announcementDialog = false">取消</el-button><el-button type="primary" :loading="announcementSaving" @click="saveAnnouncement">保存</el-button></template></el-dialog>
    <el-dialog v-model="announcementDetailDialog" title="公告详情" width="min(620px, calc(100vw - 24px))"><template v-if="announcementDetail"><div class="project-announcement-detail__meta"><span>{{ announcementDetail.stage_name || announcementDetail.stage_code }}</span><time>{{ formatDateOnly(announcementDetail.created_at) || '-' }}</time></div><h3>{{ announcementDetail.title }}</h3><p class="project-announcement-detail__content">{{ announcementPlainText(announcementDetail.content_html) }}</p></template><template #footer><el-button @click="announcementDetailDialog = false">关闭</el-button></template></el-dialog>
...
.project-tabs-shell { min-width: 0; }
.project-tabs-shell { position: relative; }
.project-section-heading--actions { justify-content: flex-end; }
.project-tabs { margin-top: 8px; }
.project-tabs .el-tabs__header { display: flex; min-height: 48px; align-items: center; gap: 4px; margin: 0 0 14px; padding: 4px; overflow-x: auto; background: var(--panel-bg); border: 1px solid var(--line); border-radius: 6px; }
.project-tabs .el-tabs__nav-wrap { width: 100%; }
.project-tabs .el-tabs__nav-wrap::after { display: none; }
.project-tabs .el-tabs__nav-scroll { overflow-x: auto; }
.project-tabs .el-tabs__nav { display: flex; gap: 4px; }
.project-tabs .el-tabs__item { display: inline-flex; min-width: 120px; height: 38px; flex: 0 0 auto; align-items: center; justify-content: center; gap: 7px; padding: 0 15px; color: var(--muted); background: transparent; border-radius: 4px; font-size: 13px; line-height: 38px; }
.project-tabs .el-tabs__item:hover { color: var(--text); background: var(--panel-muted); }
.project-tabs .el-tabs__item.is-active { color: var(--brand-strong); background: color-mix(in srgb, var(--brand) 14%, var(--panel-bg)); font-weight: 650; }
.project-tabs .el-tabs__active-bar { display: none; }
.project-tabs .el-tabs__content { padding-top: 0; }
.project-overview { display: grid; grid-template-columns: minmax(0, .8fr) minmax(0, 1fr) minmax(0, .86fr); gap: 16px; align-items: start; }
.project-overview__hero, .project-overview__schedule, .project-overview__release-calendar { min-width: 0; }
.project-overview__hero { display: grid !important; width: 100%; min-width: 0; grid-template-columns: minmax(0, 1fr) !important; grid-auto-flow: row; align-content: start; gap: 14px; overflow: hidden; padding: 16px; box-sizing: border-box; }.project-overview__hero > .project-overview__identity, .project-overview__hero > .project-overview__hero-progress, .project-overview__hero > .project-overview__facts, .project-overview__hero > .project-overview__description-copy { width: 100%; min-width: 0; grid-column: 1 !important; }.project-overview__hero .project-overview__identity { display: block; min-width: 0; }.project-overview__hero .project-overview__icon { display: none; }.project-overview__hero .project-overview__identity h2 { display: -webkit-box; margin: 4px 0 0; overflow: hidden; font-size: 19px; line-height: 1.4; overflow-wrap: anywhere; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }.project-overview__hero .project-overview__identity p { margin: 4px 0 0; color: var(--muted); font-size: 12px; }.project-overview__hero .project-overview__hero-progress { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 6px 10px; padding: 10px 0; border-top: 1px solid var(--line); border-bottom: 1px solid var(--line); box-sizing: border-box; }.project-overview__hero .project-overview__hero-progress .el-progress { width: 100%; grid-column: 1 / -1; }.project-overview__hero .project-overview__facts { display: grid; grid-template-columns: minmax(0, 1fr); gap: 10px; }.project-overview__hero .project-overview__facts div { display: grid; min-width: 0; gap: 4px; padding: 0; }.project-overview__hero .project-overview__facts div, .project-overview__hero .project-overview__facts strong { min-width: 0; overflow-wrap: anywhere; }.project-overview__hero .project-overview__facts span { color: var(--muted); font-size: 12px; }.project-overview__hero .project-overview__facts strong { font-size: 13px; font-weight: 600; text-align: left; }.project-overview__hero .project-overview__facts .ui-status-tag { justify-self: start; }.project-overview__hero .project-overview__description-copy { display: grid; gap: 5px; padding-top: 2px; border-top: 1px solid var(--line); }.project-overview__hero .project-overview__description-copy p { display: -webkit-box; margin: 0; overflow: hidden; color: var(--muted); font-size: 12px; line-height: 1.55; overflow-wrap: anywhere; -webkit-box-orient: vertical; -webkit-line-clamp: 3; }
.project-overview__announcements { display: grid; align-content: start; gap: 4px; padding: 14px; background: var(--panel-bg); border: 1px solid var(--line); border-radius: 6px; }
.project-overview__announcements-heading { display: flex; align-items: start; justify-content: space-between; gap: 8px; }
.project-overview__announcements h3 { margin: 3px 0 0; font-size: 14px; }
.project-overview__announcement { display: grid; gap: 6px; padding: 10px 0; border-top: 1px solid var(--line); }
.project-overview__announcement header { display: flex; align-items: center; gap: 6px; color: var(--muted); font-size: 11px; }
.project-overview__announcement strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 13px; }
.project-overview__announcement p { display: -webkit-box; margin: 0; overflow: hidden; color: var(--muted); font-size: 12px; line-height: 1.5; -webkit-box-orient: vertical; -webkit-line-clamp: 2; }
.project-overview__announcement { width: 100%; grid-template-columns: minmax(0, 1fr) auto; align-items: center; padding: 6px 0; color: inherit; text-align: left; background: transparent; border: 0; border-top: 1px solid var(--line); cursor: pointer; }.project-overview__announcement:hover strong { color: var(--brand-strong); }.project-overview__announcement > span { display: flex; min-width: 0; align-items: center; gap: 6px; }.project-overview__announcement strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.project-overview__announcement time { color: var(--muted); font-size: 12px; white-space: nowrap; }.project-announcement-detail__meta { display: flex; justify-content: space-between; gap: 12px; color: var(--muted); font-size: 13px; }.project-announcement-detail__content { margin: 14px 0 0; white-space: pre-wrap; overflow-wrap: anywhere; line-height: 1.7; }
.project-overview__release-calendar { display: grid; min-width: 0; align-content: start; gap: 12px; padding: 14px; background: var(--panel-bg); border: 1px solid var(--line); border-radius: 6px; }.project-overview__release-calendar > header { display: flex; min-width: 0; align-items: center; justify-content: space-between; gap: 8px; }.project-overview__release-calendar h3 { margin: 3px 0 0; font-size: 14px; }.project-overview__calendar-actions { display: flex; flex: 0 0 auto; align-items: center; gap: 2px; }.project-overview__calendar-actions strong { min-width: 58px; color: var(--text); font-size: 12px; text-align: center; }.project-overview__calendar-grid { display: grid; grid-template-columns: repeat(7, minmax(0, 1fr)); gap: 4px; }.project-overview__calendar-grid > span { color: var(--muted); font-size: 11px; text-align: center; }.project-overview__calendar-grid time { display: block; position: relative; min-width: 0; aspect-ratio: 1; padding: 4px; box-sizing: border-box; color: var(--text); font-size: 12px; }.project-overview__calendar-grid time.is-empty { color: transparent; }.project-overview__calendar-grid time.has-release { --project-release-calendar-theme: var(--brand); color: var(--project-release-calendar-theme); background: color-mix(in srgb, var(--project-release-calendar-theme) 18%, var(--panel-bg)); border: 1px solid color-mix(in srgb, var(--project-release-calendar-theme) 56%, var(--line)); border-radius: 3px; font-weight: 700; }.project-overview__calendar-title { display: block; max-width: 100%; overflow: hidden; color: var(--project-release-calendar-theme); font-size: 10px; font-weight: 650; line-height: 1.2; text-align: left; text-overflow: ellipsis; white-space: nowrap; }.project-overview__calendar-day { position: absolute; right: 4px; bottom: 3px; font-size: 12px; line-height: 1; }.project-overview__calendar-grid time.theme-system, .project-overview__calendar-grid time.theme-primary { --project-release-calendar-theme: var(--brand); }.project-overview__calendar-grid time.theme-success { --project-release-calendar-theme: var(--success); }.project-overview__calendar-grid time.theme-warning { --project-release-calendar-theme: var(--warning); }.project-overview__calendar-grid time.theme-danger { --project-release-calendar-theme: var(--danger); }.project-overview__calendar-grid time.theme-info { --project-release-calendar-theme: var(--accent); }.project-overview__calendar-grid time.theme-ocean { --project-release-calendar-theme: #147d92; }.project-overview__calendar-grid time.theme-emerald { --project-release-calendar-theme: #247a62; }.project-overview__calendar-grid time.theme-sunset { --project-release-calendar-theme: #b9503d; }.project-overview__calendar-grid time.theme-graphite { --project-release-calendar-theme: #596273; }.project-overview__calendar-grid time.theme-tech-blue { --project-release-calendar-theme: #2563eb; }.project-overview__calendar-grid time.theme-violet { --project-release-calendar-theme: #7657a6; }.project-overview__calendar-grid time.theme-amber { --project-release-calendar-theme: #b45309; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-ocean { --project-release-calendar-theme: #6ac0c7; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-emerald { --project-release-calendar-theme: #75c69f; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-sunset { --project-release-calendar-theme: #f0a18b; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-graphite { --project-release-calendar-theme: #aebaca; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-tech-blue { --project-release-calendar-theme: #69a2ff; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-violet { --project-release-calendar-theme: #c3a5ef; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-amber { --project-release-calendar-theme: #f0b45a; }.project-overview__schedule { grid-column: 1 / -1; }
.project-overview__calendar-grid time.theme-tone-1 { --project-release-calendar-theme: var(--brand); }.project-overview__calendar-grid time.theme-tone-2 { --project-release-calendar-theme: color-mix(in srgb, var(--brand) 68%, var(--success)); }.project-overview__calendar-grid time.theme-tone-3 { --project-release-calendar-theme: color-mix(in srgb, var(--brand) 58%, var(--accent)); }.project-overview__calendar-grid time.theme-tone-4 { --project-release-calendar-theme: color-mix(in srgb, var(--brand) 46%, var(--warning)); }.project-overview__calendar-grid time.theme-tone-5 { --project-release-calendar-theme: color-mix(in srgb, var(--brand) 34%, var(--danger)); }.release-calendar-theme-option { display: inline-flex; align-items: center; gap: 8px; }.release-calendar-theme-option i { width: 12px; height: 12px; border: 1px solid color-mix(in srgb, var(--text) 22%, transparent); border-radius: 3px; }.release-calendar-theme-option i.tone-1 { background: var(--brand); }.release-calendar-theme-option i.tone-2 { background: color-mix(in srgb, var(--brand) 68%, var(--success)); }.release-calendar-theme-option i.tone-3 { background: color-mix(in srgb, var(--brand) 58%, var(--accent)); }.release-calendar-theme-option i.tone-4 { background: color-mix(in srgb, var(--brand) 46%, var(--warning)); }.release-calendar-theme-option i.tone-5 { background: color-mix(in srgb, var(--brand) 34%, var(--danger)); }
.project-overview__calendar-title { position: absolute; top: 4px; right: 4px; max-width: calc(100% - 28px); text-align: right; }
.project-overview__calendar-day { top: 3px; right: auto; bottom: auto; left: 4px; }
.release-calendar-theme-picker { display: flex; align-items: center; gap: 10px; min-height: 34px; }.release-calendar-theme-swatch { display: grid; width: 28px; height: 28px; place-items: center; padding: 0; color: transparent; border: 2px solid transparent; border-radius: 4px; cursor: pointer; }.release-calendar-theme-swatch span { font-size: 14px; font-weight: 700; }.release-calendar-theme-swatch.is-selected { color: #fff; border-color: var(--text); box-shadow: 0 0 0 2px var(--panel-bg), 0 0 0 3px color-mix(in srgb, var(--text) 42%, transparent); }.release-calendar-theme-swatch:focus-visible { outline: 2px solid var(--brand); outline-offset: 2px; }.release-calendar-theme-swatch.tone-1 { background: var(--brand); }.release-calendar-theme-swatch.tone-2 { background: color-mix(in srgb, var(--brand) 68%, var(--success)); }.release-calendar-theme-swatch.tone-3 { background: color-mix(in srgb, var(--brand) 58%, var(--accent)); }.release-calendar-theme-swatch.tone-4 { background: color-mix(in srgb, var(--brand) 46%, var(--warning)); }.release-calendar-theme-swatch.tone-5 { background: color-mix(in srgb, var(--brand) 34%, var(--danger)); }
.project-overview { align-items: stretch; }.project-overview__hero { height: 100%; }.project-overview__hero .project-overview__hero-progress { padding: 4px 0; border: 0; }.project-overview__hero .project-overview__facts { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px 16px; }.project-overview__hero .project-overview__facts div:nth-child(2) { grid-column: 1 / -1; }.project-overview__announcements, .project-overview__release-calendar { height: 100%; min-height: 0; box-sizing: border-box; }
.project-announcement-list { display: grid; gap: 10px; }
.project-announcement-list article { display: grid; gap: 8px; padding: 14px; background: var(--panel-bg); border: 1px solid var(--line); border-radius: 6px; }
.project-announcement-list header { display: flex; align-items: center; justify-content: space-between; gap: 12px; }.project-announcement-list header > div { display: flex; min-width: 0; align-items: center; gap: 8px; }.project-announcement-list header strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.project-announcement-list header span, .project-announcement-list p, .project-announcement-list small { color: var(--muted); }.project-announcement-list p { margin: 0; white-space: pre-wrap; overflow-wrap: anywhere; }.project-announcement-actions { flex: 0 0 auto; flex-wrap: wrap; justify-content: flex-end; }.project-announcement-actions .el-button { margin-left: 0; }.project-announcement-editor { width: 100%; overflow: hidden; border: 1px solid var(--line); border-radius: 4px; }.project-announcement-editor .w-e-text-container { min-height: 220px; }
.release-calendar__toolbar { display: flex; gap: 10px; align-items: center; }
.release-calendar__desktop { display: grid; grid-template-columns: repeat(7, minmax(0, 1fr)); border: 1px solid var(--el-border-color-lighter); }
.release-calendar__weekday { padding: 8px; color: var(--el-text-color-secondary); background: var(--el-fill-color-light); border-right: 1px solid var(--el-border-color-lighter); border-bottom: 1px solid var(--el-border-color-lighter); font-size: 12px; font-weight: 600; text-align: center; }
.release-calendar__day { min-height: 132px; padding: 8px; border-right: 1px solid var(--el-border-color-lighter); border-bottom: 1px solid var(--el-border-color-lighter); overflow: hidden; }
.release-calendar__day.is-blank { background: var(--el-fill-color-lighter); }
.release-calendar__day-heading { display: flex; align-items: center; justify-content: space-between; min-height: 24px; margin-bottom: 6px; }
.release-calendar__day-heading > time { display: block; color: var(--el-text-color-secondary); font-size: 13px; }
.release-calendar__day-heading .el-button { margin: -4px -4px 0 0; }
.release-calendar__entry-row { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: start; gap: 2px; margin-bottom: 5px; overflow: hidden; border-left: 3px solid var(--el-color-primary); background: var(--el-fill-color-light); }
.release-calendar__entry { display: block; min-width: 0; width: 100%; text-align: left; border: 0; background: transparent; padding: 5px 7px; overflow: hidden; cursor: pointer; }
.release-calendar__entry-row .el-button { margin-top: 1px; }
.release-calendar__entry strong, .release-calendar__entry span { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.release-calendar__entry span { color: var(--el-text-color-secondary); font-size: 12px; margin-top: 2px; }
.release-calendar__mobile { display: none; }
.release-calendar__mobile-group { display: grid; grid-template-columns: 98px minmax(0, 1fr); gap: 12px; padding: 12px 0; border-bottom: 1px solid var(--el-border-color-lighter); }
.release-calendar__mobile-group > time { color: var(--el-color-primary); font-weight: 600; }.release-calendar__mobile-item { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 12px; align-items: start; padding: 0 0 12px; }.release-calendar__mobile-item:last-child { padding-bottom: 0; }.release-calendar__mobile-item p { margin: 4px 0 0; color: var(--el-text-color-secondary); white-space: pre-wrap; overflow-wrap: anywhere; }
.project-announcement-tabs { margin: 4px 0 12px; }
.project-announcement-tabs :deep(.el-tabs__nav-wrap) { overflow-x: auto; }
.project-announcement-tabs .el-tabs__content,
.project-announcement-tabs .el-tab-pane,
.project-announcement-content { min-height: 0; }
@media (max-width: 760px) {
  .project-tabs .el-tabs__header { padding: 3px; }
  .project-tabs .el-tabs__item { min-width: 106px; padding: 0 10px; }
  .project-overview { display: grid; grid-template-columns: minmax(0, 1fr); }.project-overview__announcements, .project-overview__release-calendar { margin: 0; }.project-announcement-list header { align-items: flex-start; }.project-announcement-list header > div:first-child { display: grid; align-items: start; gap: 5px; }
  .project-creation-dialog .el-col { max-width: 100%; flex: 0 0 100%; }
  .project-creation-dialog .el-date-editor { min-width: 0; }
  .project-organization-section .project-section-heading--actions { align-items: flex-end; flex-direction: row; }
  .release-calendar__toolbar { width: 100%; justify-content: space-between; }.release-calendar__toolbar .el-date-editor { min-width: 0; width: 132px; }.release-calendar__desktop { display: none; }.release-calendar__mobile { display: block; }.release-calendar__mobile-group { grid-template-columns: 84px minmax(0, 1fr); }
}
</style>
