...
import type { ProjectAnnouncement } from '../types/project'
...
const releaseCalendarForm = reactive<{ title: string; release_date: string; remark: string; theme_key: ProjectReleaseCalendarToneKey; row_version: number }>({ title: '', release_date: '', remark: '', theme_key: 'tone-1', row_version: 0 })
...
const releaseCalendarMobileGroups = computed(() => {
  ...
  releaseCalendarEntries.value.forEach(entry => { const date = String(entry.release_date).slice(0, 10); groups.set(date, [...(groups.get(date) || []), entry]) })
...
function calendarEntriesFor(date: string) { return releaseCalendarEntries.value.filter(item => String(item.release_date).slice(0, 10) === date) }
...
function releaseCalendarThemeLabel(themeKey?: ProjectReleaseCalendar['theme_key']) { return releaseCalendarThemeOptions.find(option => option.value === releaseCalendarThemeKeyForEdit(themeKey))?.label || '颜色一' }
function resetReleaseCalendarForm() { Object.assign(releaseCalendarForm, { title: '', release_date: '', remark: '', theme_key: 'tone-1', row_version: 0 }); releaseCalendarEditingId.value = null }
function openReleaseCalendarDialog(entry?: ProjectReleaseCalendar) { if (entry) { releaseCalendarEditingId.value = entry.id; Object.assign(releaseCalendarForm, { title: entry.title, release_date: String(entry.release_date).slice(0, 10), remark: entry.remark || '', theme_key: releaseCalendarThemeKeyForEdit(entry.theme_key), row_version: entry.row_version }) } else resetReleaseCalendarForm(); releaseCalendarDialog.value = true }
async function saveReleaseCalendar() { if (!selectedProject.value || !releaseCalendarForm.title.trim() || !releaseCalendarForm.release_date) { ElMessage.warning('请填写投产标题和日期'); return } releaseCalendarSubmitting.value = true; try { const payload = { title: releaseCalendarForm.title.trim(), release_date: releaseCalendarForm.release_date, remark: releaseCalendarForm.remark.trim() || undefined, theme_key: releaseCalendarForm.theme_key }; if (releaseCalendarEditingId.value) await updateProjectReleaseCalendar(selectedProject.value.id, releaseCalendarEditingId.value, { ...payload, row_version: releaseCalendarForm.row_version }); else await createProjectReleaseCalendar(selectedProject.value.id, payload); releaseCalendarDialog.value = false; await loadReleaseCalendar(); ElMessage.success('投产日历已保存') } catch (error) { ElMessage.error(apiErrorMessage(error, '投产日历保存失败')) } finally { releaseCalendarSubmitting.value = false } }
...
async function loadAnnouncements() { if (!selectedProject.value) return; announcementsLoading.value = true; announcementsError.value = ''; try { announcements.value = (await getProjectAnnouncements(selectedProject.value.id)).data.data; currentAnnouncements.value = (await getCurrentProjectAnnouncements(selectedProject.value.id)).data.data } catch (error) { announcements.value = []; currentAnnouncements.value = []; announcementsError.value = apiErrorMessage(error, '项目公告加载失败') } finally { announcementsLoading.value = false } }
...
function openAnnouncementDialog(item?: ProjectAnnouncement) { announcementEditingId.value = item?.id || null; Object.assign(announcementForm, { stage_code: item?.stage_code || selectedProject.value?.plan_stages?.[0]?.stage_code || '', title: item?.title || '', content_html: item?.content_html || '', pinned: Boolean(item?.pinned), row_version: item?.row_version || 0 }); announcementEditorHtml.value = announcementForm.content_html; announcementDialog.value = true }
...
async function saveAnnouncement() { if (!selectedProject.value || !announcementForm.stage_code || !announcementForm.title.trim() || !announcementPlainText(announcementForm.content_html)) { ElMessage.warning('请填写公告阶段、标题和正文'); return } announcementSaving.value = true; try { const payload = { stage_code: announcementForm.stage_code, title: announcementForm.title.trim(), content_html: announcementForm.content_html, pinned: announcementForm.pinned }; if (announcementEditingId.value) await updateProjectAnnouncement(selectedProject.value.id, announcementEditingId.value, { ...payload, row_version: announcementForm.row_version }); else await createProjectAnnouncement(selectedProject.value.id, payload); announcementDialog.value = false; await loadAnnouncements(); ElMessage.success('项目公告已保存') } catch (error) { ElMessage.error(apiErrorMessage(error, '项目公告保存失败')) } finally { announcementSaving.value = false } }
...
<template>
  <section class="project-page">
    ...
    <template v-else-if="selectedProject">
        ...
        <div class="project-tab-panel project-organization-panel">
          <div class="project-organization-layout">
            <aside class="project-organization-tree-panel">
              <div class="project-section-heading"><div><span class="panel-kicker">PROJECT STRUCTURE</span><h2>项目组织树</h2><p class="project-organization__hint">项目独立维护，不读取系统组织架构。</p></div><el-button v-if="canCreateMember && isOwner" type="primary" circle title="新增根组织" @click="openCreateProjectOrganization()"><el-icon><Plus /></el-icon></el-button></div>
            ...
            <div class="project-organization-maintenance">
              <section class="project-organization-section">
                <div class="project-section-heading"><div><span class="panel-kicker">PROJECT MEMBERS</span><h2>项目成员</h2><p class="project-organization__hint">将成员挂接到项目机构，并维护成员承担的项目角色。</p></div><el-button v-if="canCreateMember && isOwner" type="primary" @click="openCreateMember"><el-icon><Plus /></el-icon>添加成员</el-button></div>
      ...
      <el-tab-pane name="release-calendar"><template #label><el-icon><Calendar /></el-icon><span>投产日历</span></template>
        <div class="project-tab-panel release-calendar" v-loading="releaseCalendarLoading">
          <div class="project-section-heading"><div><span class="panel-kicker">RELEASE CALENDAR</span><h2>投产日历</h2><p>项目成员可独立维护投产安排，不关联配置管理投产窗口。</p></div><div class="release-calendar__toolbar"><el-date-picker v-model="releaseCalendarMonth" type="month" value-format="YYYY-MM" :clearable="false" /><el-button type="primary" @click="openReleaseCalendarDialog()"><el-icon><Plus /></el-icon>新增投产安排</el-button></div></div>
          ...
          <template v-else><div class="release-calendar__desktop"><span v-for="weekday in releaseCalendarWeekdays" :key="weekday" class="release-calendar__weekday">周{{ weekday }}</span><article v-for="(date, index) in releaseCalendarDays" :key="date || `blank-${index}`" class="release-calendar__day" :class="{ 'is-blank': !date }"><template v-if="date"><time>{{ Number(date.slice(-2)) }}</time><div v-for="entry in calendarEntriesFor(date)" :key="entry.id" class="release-calendar__entry-row"><button type="button" class="release-calendar__entry" @click="openReleaseCalendarDialog(entry)"><strong>{{ entry.title }}</strong><span>{{ entry.remark || '暂无备注' }}</span></button><el-tooltip content="删除投产安排" placement="top"><el-button text circle type="danger" aria-label="删除投产安排" @click.stop="removeReleaseCalendar(entry)"><el-icon><Delete /></el-icon></el-button></el-tooltip></div><el-button text size="small" @click="openReleaseCalendarDialog({ id: 0, project_id: selectedProject.id, title: '', release_date: date, remark: '', row_version: 0 })"><el-icon><Plus /></el-icon></el-button></template></article></div>
          <div class="release-calendar__mobile"><el-empty v-if="!releaseCalendarEntries.length" description="本月暂无投产安排" /><section v-for="group in releaseCalendarMobileGroups" :key="group.date" class="release-calendar__mobile-group"><time>{{ formatDateOnly(group.date) }}</time><article v-for="entry in group.entries" :key="entry.id" class="release-calendar__mobile-item"><div><strong>{{ entry.title }}</strong><p>{{ entry.remark || '暂无备注' }}</p></div><span><el-button text type="primary" @click="openReleaseCalendarDialog(entry)">编辑</el-button><el-button text type="danger" @click="removeReleaseCalendar(entry)">删除</el-button></span></article></section></div></template>
        </div>
      </el-tab-pane>
        <el-tab-pane name="announcements"><template #label><el-icon><Document /></el-icon><span>公告管理</span></template>
          <div class="project-tab-panel project-announcements" v-loading="announcementsLoading">
            <div class="project-section-heading"><div><span class="panel-kicker">PROJECT ANNOUNCEMENTS</span><h2>公告管理</h2><p>项目成员可按实施阶段发布、置顶和维护公告。</p></div><el-button type="primary" @click="openAnnouncementDialog()"><el-icon><Plus /></el-icon>发布公告</el-button></div>
        ...
        <el-tab-pane name="settings"><template #label><el-icon><Setting /></el-icon><span>项目设置</span></template>
          <div class="project-tab-panel project-settings">
            <div class="project-section-heading"><div><span class="panel-kicker">PROJECT SETTINGS</span><h2>项目设置</h2></div></div>
    ...
    <el-dialog v-model="announcementDialog" :title="announcementEditingId ? '编辑公告' : '发布公告'" width="min(760px, calc(100vw - 24px))" destroy-on-close @closed="announcementEditorClosed"><el-form label-position="top"><el-form-item label="实施阶段" required><el-select v-model="announcementForm.stage_code" style="width:100%"><el-option v-for="stage in selectedProject?.plan_stages || []" :key="stage.stage_code" :label="stage.stage_name" :value="stage.stage_code" /></el-select></el-form-item><el-form-item label="公告标题" required><el-input v-model="announcementForm.title" maxlength="128" show-word-limit /></el-form-item><el-form-item label="公告正文" required><div class="project-announcement-editor"><Toolbar :editor="announcementEditor" mode="default" /><WangEditor v-model="announcementEditorHtml" mode="default" @on-created="announcementEditorCreated" @on-change="announcementEditorChanged" /></div></el-form-item><el-form-item><el-checkbox v-model="announcementForm.pinned">置顶公告</el-checkbox></el-form-item></el-form><template #footer><el-button :disabled="announcementSaving" @click="announcementDialog = false">取消</el-button><el-button type="primary" :loading="announcementSaving" @click="saveAnnouncement">保存</el-button></template></el-dialog>
...
.project-overview__announcements { display: grid; gap: 10px; padding: 14px; background: var(--panel-bg); border: 1px solid var(--line); border-radius: 6px; }
...
.project-overview__announcement { width: 100%; grid-template-columns: minmax(0, 1fr) auto; align-items: center; padding: 10px 0; color: inherit; text-align: left; background: transparent; border: 0; border-top: 1px solid var(--line); cursor: pointer; }.project-overview__announcement:hover strong { color: var(--brand-strong); }.project-overview__announcement > span { display: flex; min-width: 0; align-items: center; gap: 6px; }.project-overview__announcement strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.project-overview__announcement time { color: var(--muted); font-size: 12px; white-space: nowrap; }.project-announcement-detail__meta { display: flex; justify-content: space-between; gap: 12px; color: var(--muted); font-size: 13px; }.project-announcement-detail__content { margin: 14px 0 0; white-space: pre-wrap; overflow-wrap: anywhere; line-height: 1.7; }
.project-overview__release-calendar { display: grid; min-width: 0; align-content: start; gap: 12px; padding: 14px; background: var(--panel-bg); border: 1px solid var(--line); border-radius: 6px; }.project-overview__release-calendar > header { display: flex; min-width: 0; align-items: center; justify-content: space-between; gap: 8px; }.project-overview__release-calendar h3 { margin: 3px 0 0; font-size: 14px; }.project-overview__calendar-actions { display: flex; flex: 0 0 auto; align-items: center; gap: 2px; }.project-overview__calendar-actions strong { min-width: 58px; color: var(--text); font-size: 12px; text-align: center; }.project-overview__calendar-grid { display: grid; grid-template-columns: repeat(7, minmax(0, 1fr)); gap: 4px; }.project-overview__calendar-grid > span { color: var(--muted); font-size: 11px; text-align: center; }.project-overview__calendar-grid time { display: grid; min-width: 0; aspect-ratio: 1; place-items: center; color: var(--text); font-size: 12px; }.project-overview__calendar-grid time.is-empty { color: transparent; }.project-overview__calendar-grid time.has-release { --project-release-calendar-theme: var(--brand); color: var(--project-release-calendar-theme); background: color-mix(in srgb, var(--project-release-calendar-theme) 18%, var(--panel-bg)); border: 1px solid color-mix(in srgb, var(--project-release-calendar-theme) 56%, var(--line)); border-radius: 3px; cursor: help; font-weight: 700; }.project-overview__calendar-grid time.theme-system, .project-overview__calendar-grid time.theme-primary { --project-release-calendar-theme: var(--brand); }.project-overview__calendar-grid time.theme-success { --project-release-calendar-theme: var(--success); }.project-overview__calendar-grid time.theme-warning { --project-release-calendar-theme: var(--warning); }.project-overview__calendar-grid time.theme-danger { --project-release-calendar-theme: var(--danger); }.project-overview__calendar-grid time.theme-info { --project-release-calendar-theme: var(--accent); }.project-overview__calendar-grid time.theme-ocean { --project-release-calendar-theme: #147d92; }.project-overview__calendar-grid time.theme-emerald { --project-release-calendar-theme: #247a62; }.project-overview__calendar-grid time.theme-sunset { --project-release-calendar-theme: #b9503d; }.project-overview__calendar-grid time.theme-graphite { --project-release-calendar-theme: #596273; }.project-overview__calendar-grid time.theme-tech-blue { --project-release-calendar-theme: #2563eb; }.project-overview__calendar-grid time.theme-violet { --project-release-calendar-theme: #7657a6; }.project-overview__calendar-grid time.theme-amber { --project-release-calendar-theme: #b45309; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-ocean { --project-release-calendar-theme: #6ac0c7; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-emerald { --project-release-calendar-theme: #75c69f; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-sunset { --project-release-calendar-theme: #f0a18b; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-graphite { --project-release-calendar-theme: #aebaca; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-tech-blue { --project-release-calendar-theme: #69a2ff; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-violet { --project-release-calendar-theme: #c3a5ef; }html[data-theme="dark"] .project-overview__calendar-grid time.theme-amber { --project-release-calendar-theme: #f0b45a; }.project-overview__release-tooltip { display: grid; max-width: 260px; gap: 7px; }.project-overview__release-tooltip strong { display: grid; gap: 2px; font-size: 12px; font-weight: 600; }.project-overview__release-tooltip small { color: var(--el-text-color-secondary); font-weight: 400; overflow-wrap: anywhere; }.project-overview__schedule { grid-column: 1 / -1; }
...
.release-calendar__day > time { display: block; color: var(--el-text-color-secondary); font-size: 13px; margin-bottom: 6px; }
