    ...
    <template v-else-if="node">
      ...
      <el-form label-position="top" size="default">
        ...
        <template v-else-if="node.type === 'CC'">
          <el-form-item v-if="projectScoped" label="抄送人来源" required><el-radio-group v-model="draft.config.templatePlaceholder" :disabled="readonly" @change="update"><el-radio-button :value="true">待配置人员</el-radio-button><el-radio-button :value="false">项目成员</el-radio-button></el-radio-group></el-form-item>
          <el-form-item label="抄送人员" required><el-tag v-if="templateScoped" effect="plain" type="info">项目配置时指定</el-tag><el-alert v-else-if="draft.config.templatePlaceholder" type="warning" :closable="false" show-icon title="待选择项目成员后发布。" /><el-select v-else v-model="draft.config.userIds" multiple filterable collapse-tags :disabled="readonly" placeholder="请选择抄送用户" @change="update"><el-option v-for="user in users" :key="user.id" :label="`${user.display_name}（${user.username}）`" :value="user.id" /></el-select></el-form-item>
        </template>
