...
const emit = defineEmits<{ 'chart-click': [payload: { name: string; seriesName: string }] }>()
...
function render() {
  ...
  chart.off('click')
  chart.on('click', params => emit('chart-click', { name: String(params.name ?? ''), seriesName: String(params.seriesName ?? '') }))
  chart.resize()
}
...
<template><div ref="chartEl" class="release-chart" :style="{ height }" role="img" :aria-label="ariaLabel" /></template>
