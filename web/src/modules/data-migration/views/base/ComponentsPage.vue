...
async function load() {
  ...
  try {
    components.value = (await listDataMigrationComponents()).data.data ?? []
...
<template>
  <main class="dm-page">
    <header class="dm-page-header">
      <div><span class="dm-eyebrow">DATA MIGRATION</span><h1>系统/组件清单</h1></div>
    ...
    <section v-else class="dm-table-shell">
      <div class="dm-row dm-head dm-component-row"><span>组件编码</span><span>组件名称</span><span>项目 ID</span><span>操作</span></div>
      <div v-for="component in components" :key="component.id" class="dm-row dm-component-row"><span>{{ component.component_code }}</span><span>{{ component.component_name }}</span><span>{{ component.project_id }}</span><button class="dm-link" type="button" :disabled="actionBusy" @click="editComponent(component)">编辑</button></div>
      <p v-if="!components.length" class="dm-state">暂无组件</p>
    </section>
  </main>
</template>
...
.dm-component-row { grid-template-columns:1fr 1.5fr 1fr .8fr; }
