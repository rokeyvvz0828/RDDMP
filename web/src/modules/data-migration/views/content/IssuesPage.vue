...
const sd = (i: IssueRecord): Record<string, any> => { if (!i.structured_data) return {}; try { return typeof i.structured_data === 'string' ? JSON.parse(i.structured_data) : i.structured_data } catch { return {} } }
    ...
    <el-tabs v-model="tab" @tab-click="(t: any) => switchTab(t.paneName)">
      <el-tab-pane label="问题清单" name="list">
        ...
        <UiDataTable :data="records" :loading="loading" row-key="id" @selection-change="(r: IssueRecord[]) => selectedIds = r.map(x => x.id)">
          ...
          <el-table-column label="关联纪要" min-width="140" show-overflow-tooltip><template #default="{ row }">{{ sd(row).relatedMeetingMinuteNames ?? '—' }}</template></el-table-column>
          <el-table-column label="问题相关表" min-width="140" show-overflow-tooltip><template #default="{ row }">{{ sd(row).relatedTableNames ?? '—' }}</template></el-table-column>
          <el-table-column label="问题相关字段" min-width="140" show-overflow-tooltip><template #default="{ row }">{{ sd(row).relatedFieldNames ?? '—' }}</template></el-table-column>
