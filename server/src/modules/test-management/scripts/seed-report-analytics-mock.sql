...
SELECT @base_id+1,@tenant_id,@domain,project_id,system_1,1,'【本地模拟】报告统计样本系统一',operator_1,operator_1,0 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+2,@tenant_id,@domain,project_id,system_2,1,'【本地模拟】报告统计样本系统二',operator_1,operator_1,0 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+3,@tenant_id,@domain,project_id,system_3,1,'【本地模拟】报告统计样本系统三',operator_1,operator_1,0 FROM tmp_tm_report_mock_context
ON DUPLICATE KEY UPDATE enabled=1,remark=VALUES(remark),updated_by=VALUES(updated_by),deleted=0;
...
SELECT @base_id+10,@tenant_id,@domain,project_id,'MOCK-R01','【本地模拟】第一轮主体测试','2026-01-05','2026-01-16','CLOSED',1,'统计样本',operator_1,operator_1,0 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+11,@tenant_id,@domain,project_id,'MOCK-R02','【本地模拟】第二轮回归测试','2026-02-02','2026-02-13','CLOSED',2,'统计样本',operator_1,operator_1,0 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+12,@tenant_id,@domain,project_id,'MOCK-R03','【本地模拟】第三轮验收测试','2026-03-02','2026-03-13','ACTIVE',3,'统计样本',operator_2,operator_2,0 FROM tmp_tm_report_mock_context
ON DUPLICATE KEY UPDATE round_name=VALUES(round_name),status=VALUES(status),updated_by=VALUES(updated_by),deleted=0;
...
SELECT @base_id+20,@tenant_id,@base_id+10,'MOCK-R01-C01','【本地模拟】第一轮第一周期','2026-01-05','2026-01-09','CLOSED',1,'统计样本',operator_1,operator_1,0 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+21,@tenant_id,@base_id+10,'MOCK-R01-C02','【本地模拟】第一轮第二周期','2026-01-10','2026-01-16','CLOSED',2,'统计样本',operator_1,operator_1,0 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+22,@tenant_id,@base_id+11,'MOCK-R02-C01','【本地模拟】第二轮第一周期','2026-02-02','2026-02-06','CLOSED',1,'统计样本',operator_1,operator_1,0 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+23,@tenant_id,@base_id+11,'MOCK-R02-C02','【本地模拟】第二轮第二周期','2026-02-07','2026-02-13','CLOSED',2,'统计样本',operator_2,operator_2,0 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+24,@tenant_id,@base_id+12,'MOCK-R03-C01','【本地模拟】第三轮第一周期','2026-03-02','2026-03-06','ACTIVE',1,'统计样本',operator_2,operator_2,0 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+25,@tenant_id,@base_id+12,'MOCK-R03-C02','【本地模拟】第三轮第二周期','2026-03-07','2026-03-13','DRAFT',2,'统计样本',operator_2,operator_2,0 FROM tmp_tm_report_mock_context
ON DUPLICATE KEY UPDATE cycle_name=VALUES(cycle_name),status=VALUES(status),updated_by=VALUES(updated_by),deleted=0;
...
FROM tmp_tm_report_mock_context ctx JOIN (SELECT 1 n UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6) numbers
ON DUPLICATE KEY UPDATE scope_name=VALUES(scope_name),updated_by=VALUES(updated_by),deleted=0;
...
FROM tmp_tm_report_mock_context ctx JOIN (
...
ON DUPLICATE KEY UPDATE case_name=VALUES(case_name),invalidated=VALUES(invalidated),invalid_reason=VALUES(invalid_reason),updated_by=VALUES(updated_by),deleted=0;
...
FROM tmp_tm_report_mock_context ctx JOIN (SELECT 1 seq UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6) cycles
...
ON DUPLICATE KEY UPDATE execution_status=VALUES(execution_status),executor_id=VALUES(executor_id),executed_at=VALUES(executed_at),updated_by=VALUES(updated_by),deleted=0;
...
FROM tmp_tm_report_mock_context ctx JOIN (SELECT 1 n UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 UNION ALL SELECT 10 UNION ALL SELECT 11 UNION ALL SELECT 12) numbers
ON DUPLICATE KEY UPDATE summary=VALUES(summary),severity=VALUES(severity),status=VALUES(status),handler_id=VALUES(handler_id),updated_by=VALUES(updated_by),deleted=0;
...
SELECT @base_id+4001,@tenant_id,@domain,project_id,'execution_rate','AT_LEAST',95,80,1,operator_1,operator_1 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+4002,@tenant_id,@domain,project_id,'case_success_rate','AT_LEAST',90,75,1,operator_1,operator_1 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+4003,@tenant_id,@domain,project_id,'executed_case_success_rate','AT_LEAST',92,80,1,operator_1,operator_1 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+4004,@tenant_id,@domain,project_id,'defect_density','AT_MOST',15,30,1,operator_1,operator_1 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+4005,@tenant_id,@domain,project_id,'defect_repair_rate','AT_LEAST',90,70,0,operator_1,operator_1 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+4006,@tenant_id,@domain,project_id,'severe_defect_count','AT_MOST',0,2,1,operator_1,operator_1 FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+4007,@tenant_id,@domain,project_id,'blocked_case_count','AT_MOST',0,3,0,operator_1,operator_1 FROM tmp_tm_report_mock_context
ON DUPLICATE KEY UPDATE qualified_threshold=VALUES(qualified_threshold),risk_threshold=VALUES(risk_threshold),enabled=VALUES(enabled),updated_by=VALUES(updated_by);
...
ON DUPLICATE KEY UPDATE report_name=VALUES(report_name),current_version_no=2,generated_by=VALUES(generated_by),generated_at=VALUES(generated_at),updated_by=VALUES(updated_by);
...
SELECT @base_id+5001,@tenant_id,@base_id+5000,1,'V2',JSON_OBJECT('title','【本地模拟】项目全周期测试报告','execution_in_progress',10,'execution_total',30),JSON_OBJECT('enabled_metrics',5,'overall','风险'),operator_1,NOW() FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+5002,@tenant_id,@base_id+5000,2,'V2',JSON_OBJECT('title','【本地模拟】项目全周期测试报告 V2','execution_in_progress',10,'execution_total',30),JSON_OBJECT('enabled_metrics',5,'overall','风险'),operator_2,NOW() FROM tmp_tm_report_mock_context
ON DUPLICATE KEY UPDATE snapshot_json=VALUES(snapshot_json),quality_snapshot_json=VALUES(quality_snapshot_json),generated_by=VALUES(generated_by),generated_at=VALUES(generated_at);
...
SELECT @base_id+6001,@tenant_id,@domain,project_id,@base_id+10,'RPT-001',JSON_OBJECT('summary',JSON_OBJECT('effective_case_total',56,'invalid_case_total',4,'execution_in_progress',10,'execution_total',30,'execution_rate',53.57)),operator_1,NOW() FROM tmp_tm_report_mock_context
UNION ALL SELECT @base_id+6002,@tenant_id,@domain,project_id,@base_id+11,'CHT-001',JSON_OBJECT('summary',JSON_OBJECT('success_rate',35.71,'defect_total',12,'defect_open',6)),operator_2,NOW() FROM tmp_tm_report_mock_context
