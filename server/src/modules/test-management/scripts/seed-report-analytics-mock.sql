...
SELECT p.id AS project_id,p.project_code,@operator_1 AS operator_1,@operator_2 AS operator_2,@system_1 AS system_1,@system_2 AS system_2,@system_3 AS system_3,@team_1 AS team_1,
...
SELECT ctx.seed_base+n,@tenant_id,@domain,ctx.project_id,CASE n WHEN 1 THEN ctx.system_1 WHEN 2 THEN ctx.system_2 ELSE ctx.system_3 END,1,CONCAT('【本地模拟】',ctx.project_code,'报告统计样本系统',ELT(n,'一','二','三')),ctx.operator_1,ctx.operator_1,0
...
SELECT ctx.seed_base+100+n,@tenant_id,@domain,ctx.project_id,CASE MOD(n,3) WHEN 0 THEN ctx.system_1 WHEN 1 THEN ctx.system_2 ELSE ctx.system_3 END,NULL,CONCAT(ctx.project_code,'-RPT-MOCK-S-',LPAD(n,2,'0')),CONCAT('【本地模拟】',ctx.project_code,'测试范围',n),'模拟菜单','ONLINE','NEW',IF(MOD(n,3)=0,'HIGH','MEDIUM'),'否',ctx.operator_1,ctx.operator_1,0
FROM tmp_tm_report_mock_context ctx CROSS JOIN (SELECT 1 n UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6) numbers
...
SELECT ctx.seed_base+1000+n,@tenant_id,@domain,ctx.project_id,CASE MOD(n,3) WHEN 0 THEN ctx.system_1 WHEN 1 THEN ctx.system_2 ELSE ctx.system_3 END,ctx.seed_base+100+MOD(n-1,6)+1,NULL,CONCAT(ctx.project_code,'-RPT-MOCK-C-',LPAD(n,3,'0')),n,CONCAT('【本地模拟】',ctx.project_code,'统计案例',n),'功能','系统',IF(MOD(n+ctx.project_variant,4)=0,'P1','P2'),IF(MOD(n+ctx.project_variant,15)=0,1,0),IF(MOD(n+ctx.project_variant,15)=0,'【本地模拟】无效案例',''),'<p>本地模拟前置条件</p>','<p>执行模拟步骤</p>','<p>得到模拟结果</p>',CONCAT('【本地模拟】',ctx.project_code,'用于验证报告统计'),ctx.operator_1,ctx.operator_1,0
...
WHERE n<=CASE ctx.project_variant WHEN 1 THEN 60 WHEN 2 THEN 52 WHEN 3 THEN 44 WHEN 4 THEN 40 ELSE 36 END ON DUPLICATE KEY UPDATE case_name=VALUES(case_name),priority=VALUES(priority),invalidated=VALUES(invalidated),invalid_reason=VALUES(invalid_reason),remark=VALUES(remark),updated_by=VALUES(updated_by),deleted=0;
...
SELECT ctx.seed_base+200+seq,@tenant_id,@domain,ctx.project_id,CASE MOD(seq,3) WHEN 0 THEN ctx.system_1 WHEN 1 THEN ctx.system_2 ELSE ctx.system_3 END,ctx.seed_base+11+FLOOR((seq-1)/2),ctx.seed_base+20+seq,NULL,CONCAT('【本地模拟】',ctx.project_code,'执行目录',seq),seq,ctx.operator_1,ctx.operator_1,0
...
FROM tmp_tm_report_mock_context ctx CROSS JOIN (SELECT ones.n+tens.n*10+1 n FROM (SELECT 0 n UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9) ones CROSS JOIN (SELECT 0 n UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5) tens) numbers
WHERE n<=CASE ctx.project_variant WHEN 1 THEN 60 WHEN 2 THEN 52 WHEN 3 THEN 44 WHEN 4 THEN 40 ELSE 36 END ON DUPLICATE KEY UPDATE execution_status=VALUES(execution_status),executor_id=VALUES(executor_id),executed_at=VALUES(executed_at),updated_by=VALUES(updated_by),deleted=0;
...
SELECT ctx.seed_base+3000+n,@tenant_id,@domain,ctx.project_id,CASE MOD(n,3) WHEN 0 THEN ctx.system_1 WHEN 1 THEN ctx.system_2 ELSE ctx.system_3 END,CONCAT(ctx.project_code,'-RPT-MOCK-D-',LPAD(n,3,'0')),ctx.project_variant*1000+n,CONCAT('【本地模拟】',ctx.project_code,'缺陷',n),'<p>本地模拟缺陷说明</p>',ctx.seed_base+11+FLOOR((n-1)/6),ctx.seed_base+21+MOD(n-1,6),'功能缺陷',ELT(MOD(n+ctx.project_variant-1,4)+1,'FATAL','SERIOUS','NORMAL','MINOR'),'HIGH','HIGH',ELT(MOD(n+ctx.project_variant-1,4)+1,'RAISED','ANALYZING','RESOLVED','CLOSED'),IF(MOD(n,2)=0,ctx.operator_1,ctx.operator_2),ctx.operator_1,NOW(),ctx.operator_1,ctx.operator_1,0
...
WHERE n<=CASE ctx.project_variant WHEN 1 THEN 8 WHEN 2 THEN 10 WHEN 3 THEN 12 WHEN 4 THEN 9 ELSE 11 END ON DUPLICATE KEY UPDATE summary=VALUES(summary),defect_serial_no=VALUES(defect_serial_no),severity=VALUES(severity),status=VALUES(status),handler_id=VALUES(handler_id),updated_by=VALUES(updated_by),deleted=0;
