...
public class TestAnalyticsController {
 ...
 @GetMapping("/reports/{id}/run") @PreAuthorize("hasAuthority('test-management:' + #domain + ':analytics')") public ApiResponse<Map<String,Object>> runSaved(@PathVariable String domain,@PathVariable long id,@RequestParam long projectId,@RequestParam(required=false) Long physicalSubsystemId,@RequestParam(required=false) Long roundId,@RequestParam(required=false) Long cycleId,@AuthenticationPrincipal AuthUser u){return ok(analytics.runSaved(domain,projectId,id,filter(physicalSubsystemId,roundId,cycleId,null,null),u));}
