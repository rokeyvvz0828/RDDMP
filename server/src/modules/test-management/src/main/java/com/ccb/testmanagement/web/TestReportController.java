...
@RestController @RequestMapping("/api/test-management/reports/{domain}") public class TestReportController {
 ...
 @GetMapping @PreAuthorize("hasAuthority('test-management:' + #domain + ':reports')") public ApiResponse<PageResult<Map<String,Object>>> list(@PathVariable String domain,@RequestParam long projectId,@RequestParam(required=false) Long physicalSubsystemId,@RequestParam(required=false) Long responsibleTeamOrgId,@RequestParam(required=false) Long specialNodeId,@RequestParam(required=false) String scopeType,@RequestParam(required=false) String keyword,@RequestParam(defaultValue="1") long page,@RequestParam(defaultValue="20") long size,@AuthenticationPrincipal AuthUser u){return ok(reports.list(domain,projectId,physicalSubsystemId,responsibleTeamOrgId,specialNodeId,scopeType,keyword,new PageQuery(page,Math.min(100,size)),u));}
 @PostMapping @PreAuthorize("hasAuthority('test-management:' + #domain + ':reports:create')") public ApiResponse<Map<String,Object>> generate(@PathVariable String domain,@RequestParam long projectId,@RequestParam(required=false) Long physicalSubsystemId,@RequestParam(required=false) Long responsibleTeamOrgId,@RequestParam(required=false) Long specialNodeId,@RequestParam(required=false) String scopeType,@RequestBody Map<String,Object> body,@AuthenticationPrincipal AuthUser u){if(specialNodeId!=null||"SPECIAL".equalsIgnoreCase(scopeType))throw new IllegalArgumentException("专项报告仅保留历史阅读和导出");return ok(reports.generate(domain,projectId,physicalSubsystemId,responsibleTeamOrgId,specialNodeId,scopeType,null,body,u));}
 @PostMapping("/{id}/regenerate") @PreAuthorize("hasAuthority('test-management:' + #domain + ':reports:create')") public ApiResponse<Map<String,Object>> regenerate(@PathVariable String domain,@PathVariable long id,@RequestParam long projectId,@RequestParam(required=false) Long physicalSubsystemId,@RequestParam(required=false) Long responsibleTeamOrgId,@RequestParam(required=false) Long specialNodeId,@RequestParam(required=false) String scopeType,@RequestBody Map<String,Object> body,@AuthenticationPrincipal AuthUser u){if(specialNodeId!=null||"SPECIAL".equalsIgnoreCase(scopeType))throw new IllegalArgumentException("专项报告仅保留历史阅读和导出");return ok(reports.generate(domain,projectId,physicalSubsystemId,responsibleTeamOrgId,specialNodeId,scopeType,id,body,u));}
