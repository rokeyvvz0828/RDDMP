...
class TestConfigurationServiceTest {
    @Mock TestConfigurationMapper mapper;
    ...
    void rejectsUnknownDomainBeforeReadingAnyMasterData() {
        TestConfigurationService service = new TestConfigurationService(new TestConfigurationRepository(mapper), new ObjectMapper(), users);
    ...
    void acceptsTesterAsAValidSystemRole() {
        TestConfigurationService service = new TestConfigurationService(new TestConfigurationRepository(mapper), new ObjectMapper(), users);
        when(mapper.enabledParticipatingSystemExists(any())).thenReturn(1L);
        when(mapper.roleExists(any())).thenReturn(0L);
        when(users.findActive(1, 1003L)).thenReturn(Optional.of(new UserDirectoryItem(1003, "tester", "测试人员", 1, "测试中心", null)));
        when(mapper.role(any())).thenReturn(Map.of("role_code", "TESTER", "role_name", "测试人员"));
