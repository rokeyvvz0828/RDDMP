...
public class ReleaseOperationsStore {
    ...
    public List<ReleaseDrillRound> findReleaseDrillRounds(long tenantId, long projectId) {
        return jdbc.query("SELECT r.id, r.project_id, r.round_no, r.round_name, r.planned_at, r.status, r.result_content, r.release_plan_id, p.plan_name, r.environment_id, NULL AS environment_name, NULL AS environment_code, NULL AS environment_type, r.row_version, r.updated_at FROM rel_release_drill_round r JOIN rel_release_plan p ON p.id = r.release_plan_id AND p.tenant_id = r.tenant_id AND p.project_id = r.project_id AND p.deleted = 0 WHERE r.tenant_id = ? AND r.project_id = ? AND r.deleted = 0 ORDER BY r.round_no, r.id", RELEASE_DRILL_ROUND, tenantId, projectId);
    }

    public Optional<ReleaseDrillRound> findReleaseDrillRound(long id, long tenantId, long projectId) {
        return first(jdbc.query("SELECT r.id, r.project_id, r.round_no, r.round_name, r.planned_at, r.status, r.result_content, r.release_plan_id, p.plan_name, r.environment_id, NULL AS environment_name, NULL AS environment_code, NULL AS environment_type, r.row_version, r.updated_at FROM rel_release_drill_round r JOIN rel_release_plan p ON p.id = r.release_plan_id AND p.tenant_id = r.tenant_id AND p.project_id = r.project_id AND p.deleted = 0 WHERE r.id = ? AND r.tenant_id = ? AND r.project_id = ? AND r.deleted = 0 FOR UPDATE", RELEASE_DRILL_ROUND, id, tenantId, projectId));
    }
    ...
    private static final RowMapper<ReleaseDrillRound> RELEASE_DRILL_ROUND = (rs, n) -> new ReleaseDrillRound(rs.getLong("id"), rs.getLong("project_id"), rs.getInt("round_no"), rs.getString("round_name"), dateTime(rs, "planned_at"), DrillStatus.valueOf(rs.getString("status")), rs.getString("result_content"), rs.getLong("release_plan_id"), rs.getString("plan_name"), rs.getLong("environment_id"), rs.getString("environment_name"), rs.getString("environment_code"), rs.getString("environment_type"), rs.getLong("row_version"), dateTime(rs, "updated_at"), List.of());
