...
public class StructuredAssetController {
    ...
    @PostMapping("/structured/{type}/import") @PreAuthorize("hasAnyAuthority('data-migration:write','data-migration:manage','system:admin')") public ApiResponse<Map<String,Object>> importAssets(@PathVariable String type, @RequestPart MultipartFile file, @AuthenticationPrincipal AuthUser user) { return ApiResponse.success(excel.importAssets(type, file, user), TraceId.getOrCreate()); }

    // ==== 内容结构化菜单专用端点（REQ-20260831-050）：/rules、/parameters。INTERMEDIATE_TABLE 为基础资料页，沿用 /structured/{type}。 ====
    @GetMapping({"/rules", "/parameters"}) public ApiResponse<List<Map<String,Object>>> contentList(HttpServletRequest request, @RequestParam(required=false) String keyword, @AuthenticationPrincipal AuthUser user) { return ApiResponse.success(structured.list(contentType(request), keyword, user), TraceId.getOrCreate()); }
