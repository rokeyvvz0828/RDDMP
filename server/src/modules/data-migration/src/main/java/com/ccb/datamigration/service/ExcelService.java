...
public class ExcelService {
    ...
    public byte[] export(String type, Long projectId, String systemCode, String keyword, AuthUser user) {
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            StringBuilder sql = new StringBuilder("SELECT doc_code AS asset_code, doc_name AS asset_name, project_id, system_code, '" + type + "' AS asset_type, CAST(structured_data AS CHAR) AS structured_data FROM " + table(type) + " WHERE tenant_id = ? AND project_id = ? AND deleted = 0");
            List<Object> args = new ArrayList<>(List.of(user.tenantId(), scope));
            if (systemCode != null && !systemCode.isBlank()) { sql.append(" AND system_code = ?"); args.add(systemCode.trim()); }
        ...
        try (Workbook workbook = WorkbookFactory.create(file.getInputStream())) {
            ...
            for (int i=1;i<=sheet.getLastRowNum();i++) {
                ...
                try {
                    ...
                    if (!systemCode.isBlank() && !exists("SELECT COUNT(*) FROM dm_component WHERE system_code = ? AND project_id = ? AND tenant_id = ? AND enabled = 1", systemCode, scope, user.tenantId())) throw new IllegalArgumentException("system_code not found or disabled");
                    ...
                    jdbc.update("INSERT INTO " + table(type) + " (id, tenant_id, project_id, system_code, doc_code, doc_name, structured_data, owner_id, created_by, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", nextId(), user.tenantId(), scope, systemCode, code, name, structuredData.isBlank() ? "{}" : structuredData, user.id(), user.id(), user.id());
