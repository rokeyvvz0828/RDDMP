...
public class ContentFileAssetService {
    ...
    public PageResult<Map<String, Object>> list(String type, Long projectId, String systemCode, String keyword,
        ...
        StringBuilder sql = new StringBuilder("SELECT a.id, a.project_id, a.system_code, ? AS asset_type, a.doc_code AS asset_code, a.doc_name AS asset_name, ")
    ...
    public Map<String, Object> create(String type, long projectId, String systemCode,
        ...
        long id = nextId();
        try {
            jdbc.update("INSERT INTO " + table + " (id, tenant_id, project_id, system_code, doc_code, doc_name, owner_id, created_by, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    id, user.tenantId(), projectId, systemCode == null || systemCode.isBlank() ? "" : systemCode.trim(), docCodes.generate(type), attachment.fileName(), user.id(), user.id(), user.id());
        ...
        return jdbc.queryForMap("SELECT id, project_id, system_code, doc_code AS asset_code, doc_name AS asset_name, owner_id, created_at, updated_at FROM " + table + " WHERE id = ? AND tenant_id = ?", id, user.tenantId());
    }
    ...
    public Map<String, Object> replace(String type, long id, String systemCode,
        ...
        Map<String, Object> current = findOptional(
                "SELECT project_id, system_code, owner_id FROM " + table + " WHERE id = ? AND tenant_id = ? AND deleted = 0",
                id, user.tenantId());
        ...
        AttachmentItem attachment = resolveAttachment(attachmentId, user);
        if (attachment.fileSize() <= 0 || attachment.fileSize() > MAX_FILE_SIZE) throw new BusinessException(ErrorCode.BAD_REQUEST, "File is empty or exceeds 50 MB");
        jdbc.update("UPDATE " + table + " SET system_code = ?, doc_name = ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND tenant_id = ? AND deleted = 0",
                nextSystemCode, attachment.fileName(), user.id(), id, user.tenantId());
        replaceMainFile(type, id, projectId, attachment, user);
    ...
    public Map<String, Object> findDeletedDetail(String type, long id, AuthUser user) {
        ...
        List<Map<String, Object>> rows = jdbc.queryForList(
                "SELECT a.id, a.project_id, a.system_code, ? AS asset_type, a.doc_code AS asset_code, a.doc_name AS asset_name, "
    ...
    private void ensureComponent(String systemCode, long projectId, AuthUser user) {
        if (jdbc.queryForObject("SELECT COUNT(*) FROM dm_component WHERE system_code = ? AND project_id = ? AND tenant_id = ? AND enabled = 1", Integer.class, systemCode, projectId, user.tenantId()) == 0) {
