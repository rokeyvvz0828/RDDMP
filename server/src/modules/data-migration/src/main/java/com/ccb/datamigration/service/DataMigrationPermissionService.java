...
public class DataMigrationPermissionService {
    private final DataMigrationPermissionRepository repository;
    private final ProjectWorkflowDirectoryService projectAccess;

    public DataMigrationPermissionService(DataMigrationPermissionRepository repository, ProjectWorkflowDirectoryService projectAccess) {
    ...
    public boolean isAdmin(AuthUser user) {
        return repository.adminPermissionCount(user.id(), user.tenantId()) > 0;
    }
    ...
    public void requireCategoryPermission(AuthUser user, String category, String action) {
        ...
        if (repository.actionPermissionCount(user.id(), user.tenantId(), permission, action) == 0) {
