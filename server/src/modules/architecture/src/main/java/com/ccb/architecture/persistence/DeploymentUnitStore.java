...
public class DeploymentUnitStore {
    ...
    private static final String UNIT_COLUMNS = """
            id, code, physical_subsystem_id, short_name, name, kind, status, current_version,
            description, remark, created_by, updated_by, created_at, updated_at, row_version
            """;
    private static final String VERSION_COLUMNS = """
            id, unit_id, version_no, short_name, name, kind, description, remark, published_by, published_at
            """;
    ...
    private static final RowMapper<DeploymentUnit> UNIT_MAPPER = (rs, rowNum) -> new DeploymentUnit(
            ...
            rs.getString("short_name"), rs.getString("name"), rs.getString("kind"), rs.getString("status"),
    ...
    private static final RowMapper<DeploymentUnitVersion> VERSION_MAPPER = (rs, rowNum) -> new DeploymentUnitVersion(
            ...
            rs.getString("short_name"), rs.getString("name"), rs.getString("kind"),
    ...
    public void insertUnit(long id, long tenantId, String code, long physicalSubsystemId, String shortName,
        ...
        jdbc.update("""
                INSERT INTO arch_deployment_unit
                    (id, tenant_id, code, physical_subsystem_id, short_name, name, kind, status,
                     current_version, description, remark, created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, 'ACTIVE', 1, ?, ?, ?, ?)
                """, id, tenantId, code, physicalSubsystemId, shortName, name, kind,
    ...
    public int updateUnitContent(long tenantId, long id, long expectedRowVersion, String shortName, String name,
        ...
        return jdbc.update("""
                ...
                SET short_name = ?, name = ?, kind = ?, description = ?, remark = ?, updated_by = ?,
                    row_version = row_version + 1
                WHERE tenant_id = ? AND id = ? AND status = 'ACTIVE' AND row_version = ?
                """, shortName, name, kind, description, remark, actorId, tenantId, id, expectedRowVersion);
    }
    ...
    public void insertVersion(long id, long tenantId, long unitId, int versionNo, String shortName, String name,
        ...
        jdbc.update("""
                INSERT INTO arch_deployment_unit_version
                    (id, tenant_id, unit_id, version_no, short_name, name, kind, description, remark, published_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, id, tenantId, unitId, versionNo, shortName, name, kind, description, remark, actorId);
    }
