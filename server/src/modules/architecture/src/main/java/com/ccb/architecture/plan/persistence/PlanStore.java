...
public class PlanStore {
    ...
    private static final String PLAN_COLUMNS = """
            p.id, p.tenant_id, p.project_id, p.plan_no, p.name, p.environment_id, p.status, p.template_id,
    ...
    private static final RowMapper<Plan> PLAN_MAPPER = (rs, rowNum) -> new Plan(
            rs.getLong("id"), rs.getLong("project_id"), rs.getString("plan_no"), rs.getString("name"),
    ...
    public void insertPlan(long tenantId, Plan plan) {
        ...
        jdbc.update("""
                INSERT INTO arch_setup_plan
                    (id, tenant_id, project_id, plan_no, name, environment_id, status, template_id,
                     template_version_no, plan_owner_user_id, planned_start, planned_end, row_version,
                     created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, plan.id(), tenantId, plan.projectId(), plan.planNo(), plan.name(), plan.environmentId(),
    ...
    public Optional<Plan> findPlan(long tenantId, long projectId, long planId) {
        return jdbc.query("SELECT " + PLAN_COLUMNS
                        + " FROM arch_setup_plan p WHERE p.tenant_id = ? AND p.project_id = ? AND p.id = ?",
                PLAN_MAPPER, tenantId, projectId, planId).stream().findFirst();
    }

    public Optional<Plan> lockPlan(long tenantId, long projectId, long planId) {
        requireTransaction();
        return jdbc.query("SELECT " + PLAN_COLUMNS
                        + " FROM arch_setup_plan p WHERE p.tenant_id = ? AND p.project_id = ? AND p.id = ? FOR UPDATE",
                PLAN_MAPPER, tenantId, projectId, planId).stream().findFirst();
    }

    public void updatePlanStatus(long tenantId, long projectId, long planId, PlanStatus status, boolean cancelled,
        ...
        jdbc.update("""
                ...
                cancelledBy == null ? 0 : cancelledBy, tenantId, projectId, planId);
    }

    public void updatePlanSchedule(long tenantId, long projectId, long planId, LocalDateTime plannedStart,
        ...
        jdbc.update("""
                ...
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, toTimestamp(plannedStart), toTimestamp(plannedEnd), tenantId, projectId, planId);
    }

    public void updatePlanActual(long tenantId, long projectId, long planId, LocalDateTime actualStart,
                                 LocalDateTime actualEnd) {
        requireTransaction();
        jdbc.update("""
                UPDATE arch_setup_plan SET actual_start = ?, actual_end = ?
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, toTimestamp(actualStart), toTimestamp(actualEnd), tenantId, projectId, planId);
    }

    // ---------- 目标快照 ----------

    public void insertTarget(long tenantId, long projectId, PlanTarget target, String addedReason) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_target
                    (id, tenant_id, project_id, plan_id, target_type, target_id, target_no, target_name,
                     target_snapshot_json, status, added_reason, created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL, 'ACTIVE', ?, ?, ?)
                """, target.id(), tenantId, projectId, target.planId(), target.targetType().name(),
                target.targetId(), target.targetNo(), target.targetName(), addedReason,
                target.id(), target.id());
    }

    public List<PlanTarget> findTargets(long tenantId, long projectId, long planId, boolean removedIncluded) {
        ...
        return jdbc.query("""
                ...
                FROM arch_plan_target WHERE tenant_id = ? AND project_id = ? AND plan_id = ?
                  AND status IN ('ACTIVE', 'REMOVED')
                """ + condition + " ORDER BY target_type ASC, target_id ASC",
                TARGET_MAPPER, tenantId, projectId, planId);
    }

    public List<PlanTarget> findActiveTargets(long tenantId, long projectId, long planId) {
        return findTargets(tenantId, projectId, planId, false);
    }

    public Optional<PlanTarget> findTarget(long tenantId, long projectId, long planId, TargetType targetType,
                                           long targetId) {
        return jdbc.query("""
                SELECT id, plan_id, target_type, target_id, target_no, target_name, status, removed_reason
                FROM arch_plan_target
                WHERE tenant_id = ? AND project_id = ? AND plan_id = ? AND target_type = ? AND target_id = ?
                """, TARGET_MAPPER, tenantId, projectId, planId, targetType.name(), targetId).stream().findFirst();
    }

    public void removeTarget(long tenantId, long projectId, long targetId, String reason, long removedBy) {
        requireTransaction();
        jdbc.update("""
                UPDATE arch_plan_target SET status = 'REMOVED', removed_reason = ?, removed_by = ?,
                    updated_by = ?
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, reason, removedBy, removedBy, tenantId, projectId, targetId);
    }

    // ---------- 环节 ----------

    public void insertStage(long tenantId, long projectId, Stage stage) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_stage
                    (id, tenant_id, project_id, plan_id, stage_no, name, sort_no, owner_user_id, planned_start,
                     planned_end, status, snapshot_json, created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, stage.id(), tenantId, projectId, stage.planId(), stage.stageNo(), stage.name(), stage.sortNo(),
    ...
    public List<Stage> findStages(long tenantId, long projectId, long planId) {
        return jdbc.query("""
                ...
                FROM arch_plan_stage WHERE tenant_id = ? AND project_id = ? AND plan_id = ?
                ORDER BY sort_no ASC, id ASC
                """, STAGE_MAPPER, tenantId, projectId, planId);
    }

    public Optional<Stage> findStage(long tenantId, long projectId, long stageId) {
        return jdbc.query("""
                SELECT id, plan_id, stage_no, name, sort_no, owner_user_id, planned_start, planned_end,
                       actual_start, actual_end, status, cancelled, cancel_reason, cancelled_by,
                       cancelled_at, snapshot_json
                FROM arch_plan_stage WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, STAGE_MAPPER, tenantId, projectId, stageId).stream().findFirst();
    }

    public void updateStageStatus(long tenantId, long projectId, long stageId, PlanStatus status, boolean cancelled,
                                  String cancelReason, Long cancelledBy, LocalDateTime cancelledAt) {
        requireTransaction();
        jdbc.update("""
                ...
                toTimestamp(cancelledAt), tenantId, projectId, stageId);
    }

    public void updateStageSchedule(long tenantId, long projectId, long stageId, LocalDateTime plannedStart,
                                    LocalDateTime plannedEnd) {
        requireTransaction();
        jdbc.update("""
                UPDATE arch_plan_stage SET planned_start = ?, planned_end = ?
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, toTimestamp(plannedStart), toTimestamp(plannedEnd), tenantId, projectId, stageId);
    }

    public void updateStageActual(long tenantId, long projectId, long stageId, LocalDateTime actualStart,
                                  LocalDateTime actualEnd) {
        requireTransaction();
        jdbc.update("""
                UPDATE arch_plan_stage SET actual_start = ?, actual_end = ?
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, toTimestamp(actualStart), toTimestamp(actualEnd), tenantId, projectId, stageId);
    }

    // ---------- 任务 ----------

    public void insertTask(long tenantId, long projectId, Task task) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_task
                    (id, tenant_id, project_id, plan_id, stage_id, task_no, name, target_type, target_id, target_no,
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, task.id(), tenantId, projectId, task.planId(), task.stageId(), task.taskNo(), task.name(),
    ...
    public Optional<Task> lockTask(long tenantId, long projectId, long taskId) {
        requireTransaction();
        return jdbc.query(taskSelect("WHERE task.tenant_id = ? AND task.project_id = ? AND task.id = ? FOR UPDATE"),
                TASK_MAPPER, tenantId, projectId, taskId).stream().findFirst();
    }

    public List<Task> findTasks(long tenantId, long projectId, Long planId, Long stageId) {
        StringBuilder where = new StringBuilder("WHERE task.tenant_id = ? AND task.project_id = ?");
    ...
    public void updateTaskExecution(long tenantId, long projectId, long taskId, TaskStatus status,
        ...
        jdbc.update("""
                ...
                updatedBy, tenantId, projectId, taskId);
    }

    public void updateTaskCancel(long tenantId, long projectId, long taskId, boolean cancelled, String cancelReason,
    ...
    public void updateTaskSchedule(long tenantId, long projectId, long taskId, LocalDateTime plannedStart,
                                   LocalDateTime plannedEnd) {
        requireTransaction();
        jdbc.update("""
                UPDATE arch_plan_task SET planned_start = ?, planned_end = ?
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, toTimestamp(plannedStart), toTimestamp(plannedEnd), tenantId, projectId, taskId);
    }

    public void updateTaskOwner(long tenantId, long projectId, long taskId, long ownerUserId, long updatedBy) {
        requireTransaction();
        jdbc.update("""
                UPDATE arch_plan_task SET owner_user_id = ?, updated_by = ?
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, ownerUserId, updatedBy, tenantId, projectId, taskId);
    }

    public void deleteTask(long tenantId, long projectId, long taskId) {
        requireTransaction();
        jdbc.update("DELETE FROM arch_plan_task WHERE tenant_id = ? AND project_id = ? AND id = ?",
                tenantId, projectId, taskId);
    }

    // ---------- 检查项 ----------

    public void insertCheckItem(long tenantId, long projectId, CheckItem item) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_check_item
                    (id, tenant_id, project_id, task_id, check_no, name, guide, sort_no, status, row_version,
                     created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, item.id(), tenantId, projectId, item.taskId(), item.checkNo(), item.name(), item.guide(),
                item.sortNo(), item.status().name(), item.rowVersion(), item.createdBy(),
                item.createdBy());
    }

    public List<CheckItem> findCheckItems(long tenantId, long projectId, long taskId) {
        return jdbc.query("""
                SELECT id, task_id, check_no, name, guide, sort_no, status, remark, completed_by,
                       ...
                       created_by
                FROM arch_plan_check_item WHERE tenant_id = ? AND project_id = ? AND task_id = ?
                ORDER BY sort_no ASC, id ASC
                """, CHECK_ITEM_MAPPER, tenantId, projectId, taskId);
    }

    public Optional<CheckItem> findCheckItem(long tenantId, long projectId, long checkItemId) {
        return jdbc.query("""
                SELECT id, task_id, check_no, name, guide, sort_no, status, remark, completed_by,
                       completed_at, cancelled, cancel_reason, cancelled_by, cancelled_at, row_version,
                       created_by
    ...
    public void updateCheckItemCompletion(long tenantId, long projectId, long checkItemId, CheckItemStatus status,
        ...
        jdbc.update("""
                ...
                tenantId, projectId, checkItemId);
    }

    public void updateCheckItemCancel(long tenantId, long projectId, long checkItemId, boolean cancelled,
                                      String cancelReason, Long cancelledBy, LocalDateTime cancelledAt) {
        requireTransaction();
        jdbc.update("""
                ...
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, cancelled ? 1 : 0, cancelReason, cancelledBy, toTimestamp(cancelledAt),
                cancelled ? "CANCELLED" : "PENDING", tenantId, projectId, checkItemId);
    }

    public void deleteCheckItem(long tenantId, long projectId, long checkItemId) {
        requireTransaction();
        jdbc.update("DELETE FROM arch_plan_check_item WHERE tenant_id = ? AND project_id = ? AND id = ?",
                tenantId, projectId, checkItemId);
    }

    // ---------- 参与人 ----------

    public void insertParticipant(long tenantId, long projectId, long id, long taskId, long userId, long createdBy) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_task_participant
                    (id, tenant_id, project_id, task_id, user_id, created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    public void deleteParticipants(long tenantId, long projectId, long taskId) {
        requireTransaction();
        jdbc.update("DELETE FROM arch_plan_task_participant WHERE tenant_id = ? AND project_id = ? AND task_id = ?",
                tenantId, projectId, taskId);
    }

    // ---------- 依赖 ----------

    public void insertDependency(long tenantId, long projectId, long id, long taskId,
                                 long predecessorId, long createdBy) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_task_dependency
                    (id, tenant_id, project_id, task_id, predecessor_id, created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """, id, tenantId, projectId, taskId, predecessorId, createdBy, createdBy);
    }

    public List<Dependency> findDependencies(long tenantId, long projectId, long taskId, boolean removedIncluded) {
        String condition = removedIncluded ? "" : " AND status = 'ACTIVE'";
        return jdbc.query("""
                SELECT id, task_id, predecessor_id, status, removed_reason
                FROM arch_plan_task_dependency
                WHERE tenant_id = ? AND project_id = ? AND task_id = ? AND status IN ('ACTIVE', 'REMOVED')
    ...
    public Optional<Dependency> findDependencyById(long tenantId, long projectId, long dependencyId) {
        return jdbc.query("""
                SELECT id, task_id, predecessor_id, status, removed_reason
                FROM arch_plan_task_dependency
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                ...
                rs.getString("removed_reason")), tenantId, projectId, dependencyId).stream().findFirst();
    }

    public void removeDependency(long tenantId, long projectId, long id, String reason, long removedBy) {
        requireTransaction();
        jdbc.update("""
                UPDATE arch_plan_task_dependency SET status = 'REMOVED', removed_reason = ?,
                    removed_by = ?, updated_by = ?
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, reason, removedBy, removedBy, tenantId, projectId, id);
    }

    public void deleteDependenciesByTask(long tenantId, long projectId, long taskId) {
        requireTransaction();
        jdbc.update("""
                DELETE FROM arch_plan_task_dependency
                WHERE tenant_id = ? AND project_id = ? AND (task_id = ? OR predecessor_id = ?)
                """, tenantId, projectId, taskId, taskId);
    }

    // ---------- 阻塞 ----------

    public void insertBlock(long tenantId, long projectId, Block block) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_block
                    (id, tenant_id, project_id, task_id, description, impact, owner_user_id, expected_resolve_at, status,
                     created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'OPEN', ?, ?)
                """, block.id(), tenantId, projectId, block.taskId(), block.description(), block.impact(),
    ...
    public Optional<Block> findBlock(long tenantId, long projectId, long blockId) {
        return jdbc.query("""
                SELECT id, task_id, description, impact, owner_user_id, expected_resolve_at, status,
                       resolved_note, resolved_by, resolved_at, created_by
    ...
    public void updateBlock(long tenantId, long projectId, long blockId, String description, String impact,
        ...
        jdbc.update("""
                UPDATE arch_plan_block SET description = ?, impact = ?, owner_user_id = ?,
                    expected_resolve_at = ?
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, description, impact, ownerUserId, toTimestamp(expectedResolveAt), tenantId,
                projectId, blockId);
    }

    public void resolveBlock(long tenantId, long projectId, long blockId, String note, long resolvedBy) {
        requireTransaction();
        jdbc.update("""
                UPDATE arch_plan_block SET status = 'RESOLVED', resolved_note = ?, resolved_by = ?
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, note, resolvedBy, tenantId, projectId, blockId);
    }

    public void deleteBlocksByTask(long tenantId, long projectId, long taskId) {
        requireTransaction();
        jdbc.update("DELETE FROM arch_plan_block WHERE tenant_id = ? AND project_id = ? AND task_id = ?",
                tenantId, projectId, taskId);
    }

    // ---------- 取消建议 ----------

    public void insertCancelSuggestion(long tenantId, long projectId, CancelSuggestion suggestion) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_check_item_cancel_suggestion
                    (id, tenant_id, project_id, check_item_id, reason, submitter_user_id, status, created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, 'PENDING', ?, ?)
                """, suggestion.id(), tenantId, projectId, suggestion.checkItemId(), suggestion.reason(),
                suggestion.submitterUserId(), suggestion.submitterUserId(), suggestion.submitterUserId());
    }

    public List<CancelSuggestion> findPendingSuggestions(long tenantId, long projectId, long checkItemId) {
    ...
    public Optional<CancelSuggestion> findSuggestion(long tenantId, long projectId, long suggestionId) {
        return jdbc.query("""
                SELECT id, check_item_id, reason, submitter_user_id, status, handled_by_user_id,
                       handled_at, handler_note
                FROM arch_plan_check_item_cancel_suggestion
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, SUGGESTION_MAPPER, tenantId, projectId, suggestionId).stream().findFirst();
    }

    public void handleSuggestion(long tenantId, long projectId, long suggestionId, String status, Long handledBy,
                                 String handlerNote) {
        requireTransaction();
        jdbc.update("""
                ...
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, status, handledBy, handlerNote, tenantId, projectId, suggestionId);
    }

    // ---------- 事件 ----------

    public void insertEvent(long tenantId, long projectId, PlanEvent event) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_event
                    (id, tenant_id, project_id, plan_id, object_type, object_id, event_type, occurred_at,
                     operator_user_id, reason, correct_of_event_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, event.id(), tenantId, projectId, event.planId(), event.objectType(), event.objectId(),
                event.eventType().name(), toTimestamp(event.occurredAt()), event.operatorUserId(),
                event.reason(), event.correctOfEventId());
    }

    public List<PlanEvent> findEvents(long tenantId, long projectId, long planId, String objectType, long objectId) {
        return jdbc.query("""
                ...
                WHERE tenant_id = ? AND project_id = ? AND plan_id = ? AND object_type = ? AND object_id = ?
                ORDER BY occurred_at ASC, id ASC
                """, EVENT_MAPPER, tenantId, projectId, planId, objectType, objectId);
    }

    public Optional<PlanEvent> findEvent(long tenantId, long projectId, long eventId) {
        return jdbc.query("""
                SELECT id, plan_id, object_type, object_id, event_type, occurred_at, operator_user_id,
                       reason, correct_of_event_id
                FROM arch_plan_event WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, EVENT_MAPPER, tenantId, projectId, eventId).stream().findFirst();
    }

    // ---------- 工单关联 ----------

    public void insertWorkOrder(long tenantId, long projectId, TaskWorkOrder workOrder) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_work_order
                    (id, tenant_id, project_id, plan_id, task_id, work_order_type, work_order_id, source,
                     created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, workOrder.id(), tenantId, projectId, workOrder.planId(), workOrder.taskId(),
                workOrder.workOrderType().name(), workOrder.workOrderId(), workOrder.source().name(),
                workOrder.id(), workOrder.id());
    }

    public List<TaskWorkOrder> findWorkOrders(long tenantId, long projectId, long taskId) {
        return jdbc.query("""
                SELECT id, task_id, plan_id, work_order_type, work_order_id, source, status
                FROM arch_plan_work_order
                WHERE tenant_id = ? AND project_id = ? AND task_id = ? AND status = 'ACTIVE'
                ORDER BY id ASC
    ...
    public Optional<TaskWorkOrder> findWorkOrder(long tenantId, long projectId, long workOrderId) {
        return jdbc.query("""
                SELECT id, task_id, plan_id, work_order_type, work_order_id, source, status
                FROM arch_plan_work_order WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, (rs, rowNum) -> new TaskWorkOrder(rs.getLong("id"), rs.getLong("task_id"),
                rs.getLong("plan_id"), WorkOrderType.valueOf(rs.getString("work_order_type")),
    ...
    public void removeWorkOrder(long tenantId, long projectId, long workOrderId, String reason, long removedBy) {
        requireTransaction();
        jdbc.update("""
                UPDATE arch_plan_work_order SET status = 'REMOVED', removed_reason = ?, removed_by = ?,
                    updated_by = ?
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, reason, removedBy, removedBy, tenantId, projectId, workOrderId);
    }

    public List<Long> openResourceRequestIds(long tenantId, long projectId, List<Long> workOrderIds) {
    ...
    public List<long[]> resourceRequestRefs(long tenantId, long projectId, List<Long> workOrderIds) {
        if (workOrderIds.isEmpty()) {
            return List.of();
        }
        String placeholders = String.join(",", java.util.Collections.nCopies(workOrderIds.size(), "?"));
    ...
    public List<Long> openNetworkWorkOrderIds(long tenantId, long projectId, List<Long> workOrderIds) {
        if (workOrderIds.isEmpty()) {
            return List.of();
        }
        String placeholders = String.join(",", java.util.Collections.nCopies(workOrderIds.size(), "?"));
    ...
    public List<PlanListRow> searchPlans(long tenantId, long projectId, Long environmentId, PlanStatus status,
        ...
        StringBuilder where = new StringBuilder("WHERE p.tenant_id = ? AND p.project_id = ?");
        List<Object> args = new ArrayList<>();
        args.add(tenantId);
        args.add(projectId);
        ...
        if (hasBlocked) {
            where.append("""
                     AND EXISTS (SELECT 1 FROM arch_plan_block b
                                JOIN arch_plan_task bt ON bt.tenant_id = b.tenant_id
                                 AND bt.project_id = b.project_id AND bt.id = b.task_id
        ...
        if (hasOverdue) {
            where.append("""
                     AND EXISTS (SELECT 1 FROM arch_plan_task ot
                                WHERE ot.tenant_id = p.tenant_id AND ot.project_id = p.project_id
                                  AND ot.plan_id = p.id
        ...
        if (hasWaived) {
            where.append("""
                     AND EXISTS (SELECT 1 FROM arch_plan_check_item wci
                                JOIN arch_plan_task wct ON wct.tenant_id = wci.tenant_id
                                 AND wct.project_id = wci.project_id AND wct.id = wci.task_id
                                WHERE wct.tenant_id = p.tenant_id AND wct.project_id = p.project_id
                                  AND wct.plan_id = p.id
        ...
        if (targetType != null && targetId != null) {
            where.append(" AND EXISTS (SELECT 1 FROM arch_plan_target pt"
                    + " WHERE pt.tenant_id = p.tenant_id AND pt.project_id = p.project_id AND pt.plan_id = p.id"
        ...
        return jdbc.query(columnsPart + """
                  (SELECT COUNT(*) FROM arch_plan_task t
                    WHERE t.tenant_id = p.tenant_id AND t.project_id = p.project_id
                      AND t.plan_id = p.id AND t.cancelled = 0) AS task_count,
                  (SELECT COUNT(*) FROM arch_plan_check_item ci
                    JOIN arch_plan_task ct ON ct.tenant_id = ci.tenant_id
                     AND ct.project_id = ci.project_id AND ct.id = ci.task_id
                    WHERE ci.tenant_id = p.tenant_id AND ci.project_id = p.project_id AND ct.plan_id = p.id
                  ...
                  (SELECT COUNT(*) FROM arch_plan_block b
                    JOIN arch_plan_task bt ON bt.tenant_id = b.tenant_id
                     AND bt.project_id = b.project_id AND bt.id = b.task_id
                    WHERE b.tenant_id = p.tenant_id AND b.project_id = p.project_id AND bt.plan_id = p.id
                      AND b.status = 'OPEN' AND bt.cancelled = 0) AS open_blocks
                  FROM arch_setup_plan p
                  JOIN arch_environment env ON env.tenant_id = p.tenant_id
    ...
    public long countPlans(long tenantId, long projectId, Long environmentId, PlanStatus status, Long ownerUserId,
    ...
    public Optional<Long> findPlanIdByTask(long tenantId, long projectId, long taskId) {
        return jdbc.query("""
                SELECT plan_id FROM arch_plan_task WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, (rs, rowNum) -> rs.getLong("plan_id"), tenantId, projectId, taskId).stream().findFirst();
    }

    /** 后台工单回写仅凭任务标识进入时，从任务行恢复可信项目。 */
    public Optional<Long> findTaskProjectId(long tenantId, long taskId) {
        return jdbc.query("SELECT project_id FROM arch_plan_task WHERE tenant_id = ? AND id = ?",
                (rs, rowNum) -> rs.getLong("project_id"), tenantId, taskId).stream().findFirst();
    }

    public record TargetRef(long id, String code, String name, String status) {
    }

    public record AlertPlan(long tenantId, long projectId, long planId, String planNo,
                            long planOwnerUserId) {
    }

    public Optional<EnvironmentRef> envReference(long tenantId, long projectId, long environmentId) {
        return jdbc.query("""
                SELECT id, code, name, status FROM arch_environment
                WHERE tenant_id = ? AND project_id = ? AND id = ?
                """, (rs, rowNum) -> new EnvironmentRef(rs.getLong("id"), rs.getString("code"),
                rs.getString("name"), rs.getString("status")), tenantId, projectId, environmentId)
                .stream().findFirst();
    }
    ...
    public List<TargetRef> listPhysicalSubsystemRefs(long tenantId, long projectId, List<Long> ids) {
        if (ids.isEmpty()) {
            return List.of();
        }
        ...
        return jdbc.query("""
                ...
                WHERE tenant_id = ? AND project_id = ? AND id IN (%s) AND deleted = 0
    ...
    public List<TargetRef> listDeploymentUnitRefs(long tenantId, long projectId, List<Long> ids) {
        if (ids.isEmpty()) {
            return List.of();
        }
        String placeholders = String.join(",", java.util.Collections.nCopies(ids.size(), "?"));
    ...
    public Map<Long, String> currentTargetNames(long tenantId, long projectId, TargetType targetType,
                                                List<Long> ids) {
        if (ids.isEmpty()) {
            return Map.of();
        }
        List<TargetRef> refs = targetType == TargetType.PHYSICAL_SUBSYSTEM
                ? listPhysicalSubsystemRefs(tenantId, projectId, ids)
                : listDeploymentUnitRefs(tenantId, projectId, ids);
    ...
    public List<AlertPlan> planIdsNeedingAlert() {
        return jdbc.query("""
                SELECT tenant_id, project_id, id, plan_no, plan_owner_user_id FROM arch_setup_plan
                WHERE status IN ('NOT_STARTED', 'IN_PROGRESS')
                """, (rs, rowNum) -> new AlertPlan(rs.getLong("tenant_id"), rs.getLong("project_id"),
                rs.getLong("id"),
                rs.getString("plan_no"), rs.getLong("plan_owner_user_id")));
    }

    public long countOverdueTasks(long tenantId, long projectId, long planId, LocalDateTime now) {
        Long count = jdbc.queryForObject("""
                SELECT COUNT(*) FROM arch_plan_task
                WHERE tenant_id = ? AND project_id = ? AND plan_id = ? AND cancelled = 0
                  AND status <> 'COMPLETED' AND planned_end IS NOT NULL AND planned_end < ?
                """, Long.class, tenantId, projectId, planId, toTimestamp(now));
        return count == null ? 0 : count;
    }

    public void insertStageDependency(long tenantId, long projectId, long id, long planId, long stageId,
                                      long predecessorStageId, long createdBy) {
        requireTransaction();
        jdbc.update("""
                INSERT INTO arch_plan_stage_dependency
                    (id, tenant_id, project_id, plan_id, stage_id, predecessor_stage_id, created_by, updated_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, id, tenantId, projectId, planId, stageId, predecessorStageId, createdBy, createdBy);
    }

    public List<Long[]> findStageDependencies(long tenantId, long projectId, long planId) {
        return jdbc.query("""
                ...
                rs.getLong("predecessor_stage_id")}, tenantId, projectId, planId);
    }

    public void insertActivity(long tenantId, long projectId, long id, String scopeType, long scopeId,
                               String objectType,
        ...
        jdbc.update("""
                INSERT INTO arch_setup_plan_activity
                    (id, tenant_id, project_id, scope_type, scope_id, object_type, object_id, action,
                     operator_user_id, reason, before_json, after_json)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, id, tenantId, projectId, scopeType, scopeId, objectType, objectId, action, operatorUserId,
