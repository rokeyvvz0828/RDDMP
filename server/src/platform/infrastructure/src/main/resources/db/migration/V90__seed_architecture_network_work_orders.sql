...
SELECT 806, 1, 800, 'menu', '网络专项工单', 'ArchitectureNetworkWorkOrders',
...
SELECT 112, 1, 'NETWORK_MANAGER', '网络办理人员', 1, 0
...
SELECT 8061, 1, 806, 'view', 'architecture:network-work-order:view', '查看网络专项工单'
...
INSERT INTO sys_menu_permission
    (id, tenant_id, menu_id, action_code, permission_code, permission_name)
SELECT 8062, 1, 806, 'apply', 'architecture:network-work-order:apply', '发起和维护网络专项工单'
WHERE EXISTS (SELECT 1 FROM sys_menu WHERE id = 806 AND tenant_id = 1 AND deleted = 0)
  AND NOT EXISTS (SELECT 1 FROM sys_menu_permission WHERE id = 8062);

INSERT INTO sys_menu_permission
    (id, tenant_id, menu_id, action_code, permission_code, permission_name)
SELECT 8063, 1, 806, 'manage', 'architecture:network-work-order:manage', '办理和审批网络专项工单'
WHERE EXISTS (SELECT 1 FROM sys_menu WHERE id = 806 AND tenant_id = 1 AND deleted = 0)
  AND NOT EXISTS (SELECT 1 FROM sys_menu_permission WHERE id = 8063);
...
WHERE permission.tenant_id = 1
  AND permission.id IN (8061, 8062, 8063)
...
INSERT IGNORE INTO sys_role_permission (role_id, permission_id, tenant_id)
SELECT DISTINCT role_permission.role_id, 8061, role_permission.tenant_id
FROM sys_role_permission role_permission
WHERE role_permission.tenant_id = 1
  AND role_permission.permission_id = 8031;

INSERT IGNORE INTO sys_role_permission (role_id, permission_id, tenant_id)
SELECT DISTINCT role_permission.role_id, 8062, role_permission.tenant_id
FROM sys_role_permission role_permission
WHERE role_permission.tenant_id = 1
...
INSERT IGNORE INTO sys_role_menu (role_id, menu_id, tenant_id)
SELECT DISTINCT role_permission.role_id, 806, role_permission.tenant_id
FROM sys_role_permission role_permission
WHERE role_permission.tenant_id = 1
  AND role_permission.permission_id IN (8061, 8062, 8063);

INSERT IGNORE INTO sys_role_menu (role_id, menu_id, tenant_id)
SELECT DISTINCT role_menu.role_id, 800, role_menu.tenant_id
FROM sys_role_menu role_menu
WHERE role_menu.tenant_id = 1
  AND role_menu.menu_id = 806;
...
WHERE NOT EXISTS (
   ...
   OR NOT EXISTS (
          ...
          WHERE role_id = 112 AND permission_id = 8061 AND tenant_id = 1
      )
   OR NOT EXISTS (
          SELECT 1 FROM sys_role_permission
          WHERE role_id = 112 AND permission_id = 8062 AND tenant_id = 1
      )
   OR NOT EXISTS (
          SELECT 1 FROM sys_role_permission
          WHERE role_id = 112 AND permission_id = 8063 AND tenant_id = 1
      )
   OR NOT EXISTS (
          SELECT 1 FROM sys_role_menu
          WHERE role_id = 112 AND menu_id = 200 AND tenant_id = 1
      )
   OR NOT EXISTS (
          SELECT 1 FROM sys_role_menu
          WHERE role_id = 112 AND menu_id = 202 AND tenant_id = 1
      )
   OR EXISTS (
          SELECT 1 FROM sys_role_menu
          WHERE role_id = 112 AND menu_id IN (201, 203, 204) AND tenant_id = 1
      )
   OR NOT EXISTS (
          SELECT 1 FROM sys_user_role
          WHERE user_id = 1 AND role_id = 112 AND tenant_id = 1
      )
   OR NOT EXISTS (
          SELECT 1 FROM sys_role_permission
          WHERE role_id = 1 AND permission_id = 8063 AND tenant_id = 1
      )
   OR NOT EXISTS (
          SELECT 1
          FROM wf_definition
