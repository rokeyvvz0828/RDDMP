-- 文件：server/src/platform/infrastructure/src/main/resources/db/migration/V52__business_day_management.sql
-- 说明：REQ-20260821-037 营业日管理正式业务能力、单一 Sidebar 入口与操作权限迁移。
-- 用途：补偿 V51 菜单结构，初始化细粒度权限，并创建环境、日历、需求和业务审计表。
-- 作者：hengguan
...
SET menu_type = 'menu',
    ...
    status = 1,
    deleted = 0
WHERE tenant_id = 1 AND id = 614;

DELETE rp
FROM sys_role_permission rp
JOIN sys_menu_permission p ON p.id = rp.permission_id AND p.tenant_id = rp.tenant_id
WHERE p.tenant_id = 1 AND p.menu_id IN (648, 649, 650, 651);

DELETE FROM sys_role_menu
WHERE tenant_id = 1 AND menu_id IN (648, 649, 650, 651);

UPDATE sys_menu
SET visible = 0, status = 0, deleted = 1
WHERE tenant_id = 1 AND id IN (648, 649, 650, 651) AND deleted = 0;

INSERT IGNORE INTO sys_menu_permission
    (id, tenant_id, menu_id, action_code, permission_code, permission_name)
VALUES
    (6141, 1, 614, 'read', 'test-management:business-day:access', '查看'),
    (6142, 1, 614, 'create', 'test-management:business-day:access:create', '新增'),
    (6143, 1, 614, 'update', 'test-management:business-day:access:update', '修改'),
    (6144, 1, 614, 'delete', 'test-management:business-day:access:delete', '删除'),
    (6145, 1, 614, 'import', 'test-management:business-day:access:import', '导入'),
    (6146, 1, 614, 'export', 'test-management:business-day:access:export', '导出'),
    (6147, 1, 614, 'review', 'test-management:business-day:access:review', '评审');

INSERT IGNORE INTO sys_role_menu (role_id, menu_id, tenant_id) VALUES (1, 614, 1);

INSERT IGNORE INTO sys_role_permission (role_id, permission_id, tenant_id)
SELECT 1, id, 1 FROM sys_menu_permission WHERE tenant_id = 1 AND menu_id = 614 AND status = 1;
