...
| 基础设施 | Docker Compose、MinIO、JWT |

## 目录结构

~~~text
ccb-web/             Vue 前端应用、路由、主题、共享组件和页面
ccb-boot/            Spring Boot 启动模块和统一 Web 配置
ccb-common/          通用响应、分页、租户和基础模型
ccb-infrastructure/  数据库迁移、存储和基础设施适配
ccb-security/        JWT 认证、授权和登录审计
ccb-system/          组织、用户、角色、菜单、参数和系统管理
ccb-workflow/        流程设计模型、BPMN 编译和 Flowable 运行服务
ccb-ai/              AI 提供商、模型、路由和执行服务
docs/                数据库、模块接入契约、设计和实施计划
.ai-control/         工程控制账本、基线、模型、任务和验证证据
~~~
...
docker ps --filter "name=ccb-platform-mysql"
~~~

项目默认使用容器名 ccb-platform-mysql、数据库 ccb_platform、端口 3306。首次启动后，后端会通过 Flyway 自动执行迁移。

### 3. 启动 MinIO

头像和对象存储功能需要 MinIO。若本机还没有运行 MinIO，可执行：

~~~powershell
docker run -d --name ccb-platform-minio -p 9000:9000 -p 9001:9001 -e MINIO_ROOT_USER=ccb-minio -e MINIO_ROOT_PASSWORD=change-this-minio-password -v ccb-platform-minio-data:/data quay.io/minio/minio server /data --console-address ":9001"
~~~
...
数据库迁移位于 ccb-infrastructure/src/main/resources/db/migration。V23__database_comments.sql 和 V24__metadata_table_comments.sql 维护中文表注释、字段注释和元数据表注释。
...
1. 前端使用 ccb-web/src/components/ui 下的共享组件和语义主题变量，不在业务页面重复定义颜色、表格、抽屉和状态样式。
...
.ai-control/ 是本项目的工程控制账本，记录需求基线、系统模型、受控任务、执行证据、独立观测、反馈和收敛结果。后续任务开始时应先阅读 .ai-control/state.json，确认当前阶段和未关闭反馈，再按账本约束推进，不要绕过账本直接修改阶段状态。

账本的核心恢复顺序：

~~~text
读取 .ai-control/state.json
  -> 确认当前阶段与目标
